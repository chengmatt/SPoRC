...
- Newton refinement in `fit_model` now takes its Hessian from the AD tape (`obj$he`) instead of finite differencing the gradient with `optimHess`, which needed one gradient evaluation per parameter. Random-effects models continue to use `optimHess`, since RTMB does not implement a tape Hessian when random effects are present. Newton refinement now also stops early if the Hessian comes back non-finite, which can happen on models that have not converged, where second derivatives are undefined at parameter values the objective and gradient still evaluate at; previously a non-finite Hessian passed NaN through `solve()` without erroring and left `optim$par` and `optim$objective` as NaN.
