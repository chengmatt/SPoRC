...
Get_Init_NAA <- function(init_age_strc,
    ...
    for(p in 1:n_pop) {
      for(r in 1:n_regions) {
        for(s in 1:n_sexes) {
          ...
          Init_NAA[p,r,,s] = c(R0_r[p,r] * sexratio[p,r,s], R0_r[p,r] * sexratio[p,r,s] * exp(-tmp_cumsum_Z))
        } # end s loop
      } # end r loop
    }
      ...
      for(p in 1:n_pop) {
        for(s in 1:n_sexes) {
          for(seas in 1:n_seas) {
            ...
            if(seas == 1) Init_NAA[p,,1,s] = R0_r[p,] * sexratio[p,,s]
      ...
      for(i in 1:n_ages) {
        for(s in 1:n_sexes) {
          for(seas in 1:n_seas) {
            ...
            if(seas == 1) Init_NAA[p,,1,s] = R0_r[p,] * sexratio[p,,s] # initialize recruitment
  ...
  for(p in 1:n_pop) {
    # Overwrite first age
    for(s in 1:n_sexes) NAA[p,,1,s] <- R0_r[p,] * sexratio[p,,s]
