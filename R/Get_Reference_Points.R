...
global_SPR <- function(pars,
  ...
  for(p in 1:n_pop) {
    for (j in 2:(n_ages - 1)) {
      for (seas in 1:n_seas) {
        ...
        if (seas == spawn_seas) {
          SB_age[1,p,, j - 1] = tmp_unfished * WAA[p,, spawn_seas, j - 1] * MatAA[p,, spawn_seas, j - 1] * exp(-t_spawn * natmort[p,, j - 1] * seasdur[seas])
          SB_age[2,p,, j - 1] = tmp_fished * WAA[p,, spawn_seas, j - 1] * MatAA[p,, spawn_seas, j - 1] * exp(-t_spawn *
  ...
  for(p in 1:n_pop) {
    tmp_unfished[p,] = as.vector(tmp_unfished[p,] %*% Movement[p,,, spawn_seas, n_ages - 1])
    tmp_fished[p,]   = as.vector(tmp_fished[p,] %*% Movement[p,,, spawn_seas, n_ages - 1])
  }
  ...
  ## Plus group spawning biomass
  for(p in 1:n_pop) {
    tmp_unfished[p,] = as.vector(tmp_unfished[p,] %*% Movement[p,,, spawn_seas, n_ages])
    tmp_fished[p,]   = as.vector(tmp_fished[p,] %*% Movement[p,,, spawn_seas, n_ages])
  }

  SB_age[1,,,n_ages] = tmp_unfished * WAA[,,spawn_seas,n_ages] * MatAA[,,spawn_seas,n_ages] * exp(-t_spawn * natmort[,,n_ages] * seasdur[spawn_seas])
  SB_age[2,,,n_ages] = tmp_fished  * WAA[,,spawn_seas,n_ages] * MatAA[,,spawn_seas,n_ages] * exp(-t_spawn * (natmort[,,n_ages] * seasdur[spawn_seas] + apply(F_fract_flt[,spawn_seas,,drop=F] * F_x * fish_sel[,n_ages,,drop=F], 1, sum)))
...
#'   SB_unfished_mat = matrix(0, n_pop, n_regions, n_regions)  # unfished spawning biomass per recruit
#'   SB_fished_mat = matrix(0, n_pop, n_regions, n_regions) # fished spawning biomass per recruit
#'
...
#'     B0_r[d] = sum(SB_unfished_mat[,d] * R0 * rec_region_prop)
...
local_BH_Fmsy <- function(pars, data) {
    ...
    for(seas in 1:n_seas) {
      ...
      for(p in 1:n_pop) {
        for(o in 1:n_regions) {
          ...
          if(seas == spawn_seas) {
            for(d in 1:n_regions) {
              SB_age[1,p,o,d,j-1] = tmp_unfished[d] * WAA[p,d,spawn_seas,j-1] * MatAA[p,d,spawn_seas,j-1] *
                exp(-(t_spawn * natmort[p,d,j-1] * seasdur[spawn_seas]))
              SB_age[2,p,o,d,j-1] =
                tmp_fished[d] * WAA[p,d,spawn_seas,j-1] * MatAA[p,d,spawn_seas,j-1] *
  ...
  if(n_pop == 1 && n_regions > 1) {

    SB_fished_mat_n   <- SB_fished_mat[1,,]
    SB_unfished_mat_n <- SB_unfished_mat[1,,]
    for(o in 1:n_regions) {
      if(rec_region_prop[1,o] > 0) {
        SB_fished_mat_n[o,]   <- SB_fished_mat_n[o,]   / rec_region_prop[1,o]
        SB_unfished_mat_n[o,] <- SB_unfished_mat_n[o,] / rec_region_prop[1,o]
      }
    }

    A = 4 * h[1,] * rec_region_prop[1,] * R0[1]
    B = rep(0, n_regions)
    for(d in 1:n_regions) B[d] = (1 - h[1,d]) * sum(SB_unfished_mat_n[,d] * rec_region_prop[1,] * R0[1])
    C = 5 * h[1,] - 1
    Req_o = R0[1] * rec_region_prop[1,]

    for(nit in 1:newton_steps) {
      x_vec    = as.numeric(t(SB_fished_mat_n) %*% Req_o)
      g_vec    = A * x_vec / (B + C * x_vec)
      iter_vec = Req_o - g_vec
      dg_dxk   = (A * B) / (B + C * x_vec)^2
      J        = diag(n_regions)
      for(d in 1:n_regions) J[d,] = J[d,] - dg_dxk[d] * SB_fished_mat_n[,d]
      Req_o    = Req_o - solve(J, as.vector(iter_vec))
    }

    x_vec_eq   <- as.numeric(t(SB_fished_mat_n)   %*% Req_o)
    x_vec_eq_0 <- as.numeric(t(SB_unfished_mat_n) %*% (R0[1] * rec_region_prop[1,]))
    SB = SB0 = array(0, dim = c(n_pop, n_regions))
    SB[1,]  <- x_vec_eq
    SB0[1,] <- x_vec_eq_0
    ...
    for(d in 1:n_regions) {
      tmp = 0 # define temp variable
      for(seas in 1:n_seas) for(o in 1:n_regions) tmp = tmp + sum(CAA[1,o, d, seas, ] * WAA[1,d, seas, ]) * Req_o[o] # get yield to destination
      Yield_r[d] = tmp
    } # end d loop
    ...
    #   SB[1, r]  <- sum(SB_fished_mat[1,,r]   * Req_o)                        # Req_o is [n_regions]
    #   SB0[1, r] <- sum(SB_unfished_mat[1,,r] * R0[1] * rec_region_prop[1,])
    # }
...
Get_Reference_Points <- function(data,
  ...
  if(type == 'multi_region') {
    ...
    if(what == 'global_BH_MSY') {
      ...
      pop_b_ref_pt[1,] <- b_ref_pt[1,] <- tmp_obj$rep$Bmsy * rep$rec_region_prop
      virgin_pop_b_ref_pt[1,1] <- virgin_b_ref_pt[1,1] <- sum(tmp_obj$rep$B0 * rep$rec_region_prop)
    }
    ...
    if(what == 'local_BH_MSY') {
      ...
      # Recruitment options
      data_list$do_recruits_move <- data$do_recruits_move # whether recruits move
      data_list$rec_region_prop <- array(rep$rec_region_prop, dim = c(data$n_pop, n_regions)) # recruitment proportions
      data_list$sex_ratio_f <- sex_ratio_f # recruitment sex ratio to use
      data_list$rec_seas_prop <- array(rep$rec_seas_prop, dim = c(data$n_pop, data$n_seas)) # seasonal recruitment
      data_list$h <- array(rep$h_trans, dim = c(n_pop, n_regions)) # steepness
      ...
      if(n_pop > 1) {
        ...
        for(r in 1:n_regions) {
          b_ref_pt[1, r]        <- tmp_obj$rep$SB[1, r]  # already absolute
          virgin_b_ref_pt[1, r] <- tmp_obj$rep$SB0[1, r]
        }
        pop_b_ref_pt[1, 1]        <- sum(b_ref_pt)
        virgin_pop_b_ref_pt[1, 1] <- sum(virgin_b_ref_pt)
      }
