...
#' \code{$data$SrvAgeComps_pop_Type} and \code{$data$SrvAgeComps_pop_LikeType}.
...
#'
#' @param input_list Named list with \code{$data}, \code{$par}, and \code{$map}
#'   sublists. Requires \code{SrvAgeComps_pop_Type}, \code{SrvAgeComps_pop_LikeType},
...
do_SrvAge_pop_theta_mapping <- function(input_list) {
  ...
  for(p in 1:input_list$data$n_pop) {
    for(f in 1:input_list$data$n_srv_fleets) {
      ...
      # get unique survey comp types
      srvage_comp_type <- unique(input_list$data$SrvAgeComps_pop_Type[,f])

      # If aggregated (ages)
      if(any(srvage_comp_type == 0) && input_list$data$SrvAgeComps_pop_LikeType[f] != 0) {
      ...
      for(r in 1:input_list$data$n_regions) {
        for(s in 1:input_list$data$n_sexes) {
          ...
          # if split by sex and region
          if(any(srvage_comp_type == 1) && input_list$data$SrvAgeComps_pop_LikeType[f] != 0) {
            map_SrvAge_pop_theta[p,r,s,f] <- counter_srvage
            counter_srvage <- counter_srvage + 1 # split by sex and region
          }

          # joint by sex, split by region
          if(any(srvage_comp_type == 2) && input_list$data$SrvAgeComps_pop_LikeType[f] != 0 && s == 1) {
      ...
      # If we are using a multinomial or there aren't any age comps for a given fleet
      if(input_list$data$SrvAgeComps_pop_LikeType[f] == 0 || sum(input_list$data$UseSrvAgeComps_pop[p,,,,f]) == 0) {
...
#' overdispersion) based on the composition type and likelihood specified in
#' \code{$data$SrvLenComps_pop_Type} and \code{$data$SrvLenComps_pop_LikeType}.
#' Parameters are mapped to \code{NA} for fleets using multinomial likelihoods
#' (\code{LikeType == 0}) or with no observed len compositions.
#'
#' @param input_list Named list with \code{$data}, \code{$par}, and \code{$map}
#'   sublists. Requires \code{SrvLenComps_pop_Type}, \code{SrvLenComps_pop_LikeType},
...
do_SrvLen_pop_theta_mapping <- function(input_list) {
  ...
  for(p in 1:input_list$data$n_pop) {
    for(f in 1:input_list$data$n_srv_fleets) {
      ...
      # get unique survey comp types
      srvlen_comp_type <- unique(input_list$data$SrvLenComps_pop_Type[,f])

      # If aggregated (lens)
      if(any(srvlen_comp_type == 0) && input_list$data$SrvLenComps_pop_LikeType[f] != 0) {
      ...
      for(r in 1:input_list$data$n_regions) {
        for(s in 1:input_list$data$n_sexes) {
          ...
          # if split by sex and region
          if(any(srvlen_comp_type == 1) && input_list$data$SrvLenComps_pop_LikeType[f] != 0) {
            map_SrvLen_pop_theta[p,r,s,f] <- counter_srvlen
            counter_srvlen <- counter_srvlen + 1 # split by sex and region
          }

          # joint by sex, split by region
          if(any(srvlen_comp_type == 2) && input_list$data$SrvLenComps_pop_LikeType[f] != 0 && s == 1) {
      ...
      if(input_list$data$SrvLenComps_pop_LikeType[f] == 0 || sum(input_list$data$UseSrvLenComps_pop[p,,,,f]) == 0) {
...
#' \code{SrvAgeComps_pop_LikeType} is in \code{c(3, 4)}.
...
do_SrvAge_pop_corr_pars_mapping <- function(input_list) {
  ...
  for(p in 1:input_list$data$n_pop) {
    for(f in 1:input_list$data$n_srv_fleets) {
      ...
      # Aggregated Correlation Parameters
      if(any(srvage_comp_type == 0) && input_list$data$SrvAgeComps_pop_LikeType[f] != 0) {
        if(input_list$data$SrvAgeComps_pop_LikeType[f] == 3) {
      ...
      for(r in 1:input_list$data$n_regions) {
        for(s in 1:input_list$data$n_sexes) {
          ...
          if(any(srvage_comp_type == 2) && input_list$data$SrvAgeComps_pop_LikeType[f] != 0 && s == 1) {
            ...
            # 2dar1 correlation
            if(input_list$data$SrvAgeComps_pop_LikeType[f] == 4) {
              for(i in 1:2) {
                if(i == 2 && input_list$data$n_sexes == 1) next # skip if we only have 1 sex
...
#'
#' Parameters are activated only when
#' \code{SrvLenComps_pop_LikeType} is in \code{c(3, 4)}.
#' These correspond to the 1D and 2D logistic-normal likelihoods.
#' All other likelihoods map correlation parameters to \code{NA}.
#'
#' For the 2D logistic-normal (\code{LikeType == 4}), both trailing elements of
...
do_SrvLen_pop_corr_pars_mapping <- function(input_list) {
  ...
  for(p in 1:input_list$data$n_pop) {
    for(f in 1:input_list$data$n_srv_fleets) {
      ...
      # Aggregated Correlation Parameters
      if(any(srvlen_comp_type == 0) && input_list$data$SrvLenComps_pop_LikeType[f] != 0) {
        if(input_list$data$SrvLenComps_pop_LikeType[f] == 3) {
      ...
      for(r in 1:input_list$data$n_regions) {
        for(s in 1:input_list$data$n_sexes) {
          ...
          if(any(srvlen_comp_type == 2) && input_list$data$SrvLenComps_pop_LikeType[f] != 0 && s == 1) {
            ...
            # 2dar1 correlation
            if(input_list$data$SrvLenComps_pop_LikeType[f] == 4) {
              for(i in 1:2) {
                if(i == 2 && input_list$data$n_sexes == 1) next # skip if we only have 1 sex
...
#' the TMB/RTMB factor map for \code{srv_fixed_sel_pars}
...
#'
#' @return The input \code{input_list} with \code{$map$srv_fixed_sel_pars}
...
#'
#' @keywords internal
do_srv_fixed_sel_pars_mapping <- function(input_list, srv_fixed_sel_pars_spec, bins, srv_sel_nonpar_est_bins) {

  # Initialize counter and mapping array for fixed effects survey selectivity
  srv_fixed_sel_pars_counter <- 1
  map_srv_fixed_sel_pars <- input_list$par$srv_fixed_sel_pars
  map_srv_fixed_sel_pars[] <- NA

  for(f in 1:input_list$data$n_srv_fleets) {

    # Validate Options
    if(!srv_fixed_sel_pars_spec[f] %in% c("est_all", "est_shared_r", "est_shared_r_s", "fix", "est_shared_s", 'fix_srv_sel_input') &&
    ...
    # check fixed selex options
    if(input_list$data$use_fixed_srv_sel[f] == 1 && stringr::str_detect(srv_fixed_sel_pars_spec[f], 'est'))
      stop("use_fixed_srv_sel has 1s for a given fleet, but srv_fixed_sel_pars_spec is specified at an est variant.")
    if(input_list$data$use_fixed_srv_sel[f] == 0 && srv_fixed_sel_pars_spec[f] == 'fix_srv_sel_input')
      stop("use_fixed_srv_sel has 0s for a given fleet, but srv_fixed_sel_pars_spec is specified at fix_srv_sel_input")
        ...
        for(s in 1:input_list$data$n_sexes) {
          for(b in 1:length(srvsel_blocks_tmp)) {
            ...
            if(sel_model_this_block == 5) {
              ...
              if(is.null(srv_sel_nonpar_est_bins)) stop("Non-parametric survey selectivtiy specified, but srv_sel_nonpar_est_bins is NULL. Please specify bins!")

              bin_groups <- srv_sel_nonpar_est_bins[[f]][[b]]
              ...
              if(any(all_bins < 1) || any(all_bins > bins))
                stop("srv_sel_nonpar_est_bins[[", f, "]][[", b, "]] contains indices outside 1:", bins)
              if(length(all_bins) != length(unique(all_bins)))
                stop("srv_sel_nonpar_est_bins[[", f, "]][[", b, "]] has duplicate bin indices")
            }
            ...
            for(i in 1:max_sel_pars) {
              ...
              if(srv_fixed_sel_pars_spec[f] == "est_all") {
                for(bi in group_bins) map_srv_fixed_sel_pars[r,bi,b,s,f] <- srv_fixed_sel_pars_counter
              ...
              if(srv_fixed_sel_pars_spec[f] == 'est_shared_r' && r == 1) {
                for(rr in 1:input_list$data$n_regions) {
                  if(srvsel_blocks_tmp[b] %in% input_list$data$srv_sel_blocks[rr,,f]) {
                    for(bi in group_bins) map_srv_fixed_sel_pars[rr, bi, b, s, f] <- srv_fixed_sel_pars_counter
                  } # end if
                } # end rr loop
                srv_fixed_sel_pars_counter <- srv_fixed_sel_pars_counter + 1
              } # end if
              ...
              if(srv_fixed_sel_pars_spec[f] == 'est_shared_s' && s == 1) {
                for(ss in 1:input_list$data$n_sexes) {
                  for(bi in group_bins) map_srv_fixed_sel_pars[r, bi, b, ss, f] <- srv_fixed_sel_pars_counter
                } # end ss loop
                srv_fixed_sel_pars_counter <- srv_fixed_sel_pars_counter + 1
              } # end if
              ...
              if(srv_fixed_sel_pars_spec[f] == 'est_shared_r_s' && r == 1 && s == 1) {
                for(rr in 1:input_list$data$n_regions) {
                  for(ss in 1:input_list$data$n_sexes) {
                    if(srvsel_blocks_tmp[b] %in% input_list$data$srv_sel_blocks[rr,,f]) {
                      for(bi in group_bins) map_srv_fixed_sel_pars[rr, bi, b, ss, f] <- srv_fixed_sel_pars_counter
                    } # end if
                  } # end ss loop
                } #end rr loop
                srv_fixed_sel_pars_counter <- srv_fixed_sel_pars_counter + 1
    ...
    # fix all parameters
    if(srv_fixed_sel_pars_spec[f] %in% c("fix", "fixed_srv_sel_input")) map_srv_fixed_sel_pars[,,,,f] <- NA
  ...
  # input into mapping list
  input_list$map$srv_fixed_sel_pars <- factor(map_srv_fixed_sel_pars)
...
#'
#' @keywords internal
do_srvsel_pe_pars_mapping <- function(input_list, srvsel_pe_pars_spec, corr_opt_semipar, bins) {
  ...
  for(f in 1:input_list$data$n_srv_fleets) {
    ...
    for(r in 1:input_list$data$n_regions) {
      ...
      if(input_list$data$cont_tv_srv_sel[r,f] == 0 || (sum(input_list$data$UseSrvIdx[r,,,f]) == 0 && sum(input_list$data$UseSrvIdx_pop[,r,,,f] == 0))) {
        ...
        if(unique(input_list$data$srv_sel_model[r,,f]) == 5) max_sel_pars <- bins # non-parametric selectivity
        if(unique(input_list$data$srv_sel_model[r,,f]) %in% c(6,7)) max_sel_pars <- 3 # logistic selectivity w/ asmyptote
        ...
        for(s in 1:input_list$data$n_sexes) {
          ...
          if(input_list$data$cont_tv_srv_sel[r,f] %in% c(3,4,5)) {
            ...
            if(input_list$data$cont_tv_srv_sel[r,f] %in% c(3,4) && input_list$data$srv_selex_type == 1) stop("Cohort-based selectivity deviations are specified, but selectivity is specified as length-based. Please choose another deviation form!")
...
#'   \code{$data$n_proj_yrs_devs}, and \code{$data$srv_selex_type}.
...
#' @param bins Number of seletivity bins
#'
...
#'
#' @keywords internal
do_srvsel_devs_mapping <- function(input_list, srv_sel_devs_spec, srvsel_devs_shared_bins, bins) {
...
Setup_Mod_Srvsel_and_Q <- function(input_list,
  ...
  input_list <- do_srv_fixed_sel_pars_mapping(input_list, srv_fixed_sel_pars_spec, bins, srv_sel_nonpar_est_bins)
  input_list <- do_srv_q_mapping(input_list, srv_q_spec)
  input_list <- do_srvsel_pe_pars_mapping(input_list, srvsel_pe_pars_spec, corr_opt_semipar, bins)
  input_list <- do_srvsel_devs_mapping(input_list, srv_sel_devs_spec, srvsel_devs_shared_bins, bins)
