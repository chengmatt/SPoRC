...
#
# Spawning biomass per recruit solvers: single_region_SPR for one region at a time, global_SPR for a
# single fishing mortality shared across regions.

#' Compute SPR reference point for a single-region or non-spatial model
#'
#' The spawning potential ratio at a trial fishing mortality \eqn{F_x}, returned as
#' the squared penalty \eqn{100 (SPR - SPR_x)^2} the outer optimizer minimizes to
#' find \eqn{F_{SPR_x}}. Several populations are covered through stray rates, but
...
#' \code{dmr} of discards die, and the surviving \code{(1 - dmr)} keeps ageing and
#' spawning.
#'
#' @param pars Named list of RTMB parameters, holding \code{log_F_x}, the
#'   log-scale trial fishing mortality.
#' @param data Named list of RTMB data: the dimensions \code{n_pop}, \code{n_ages}
#'   and \code{n_seas}; \code{seasdur} \code{[n_seas]}; \code{spawn_seas} and
#'   \code{t_spawn}; \code{F_fract_flt} \code{[n_seas, n_fish_fleets]};
#'   \code{fish_sel} and \code{ret_sel} \code{[n_pop, n_seas, n_ages,
#'   n_fish_fleets]}, the female selectivity and the retained fraction of it;
#'   \code{dmr} \code{[n_seas, n_fish_fleets]}, the fraction of discards that die;
#'   \code{natmort} \code{[n_pop, n_ages]}; \code{WAA} and \code{MatAA}
#'   \code{[n_pop, n_seas, n_ages]}; \code{sex_ratio_f} \code{[n_pop]};
#'   \code{rec_seas_prop} \code{[n_pop, n_seas]}; \code{stray_rate} and
#'   \code{natal_region} \code{[n_pop]}; \code{n_pop_in_region}
#'   \code{[n_regions]}; and the target \code{SPR_x}.
#'
#' @return Numeric scalar, the squared penalty \eqn{(SPR - SPR_x)^2}, zero at
#'   \eqn{F = F_{SPR_x}}.
#'
  ...
  for(p in 1:n_pop) {
    for (j in 2:(n_ages - 1)) {
      for (seas in 1:n_seas) {
        ...
        M_seas = natmort[p,seas, j - 1] * seasdur[seas]
...
#'
#' Spatially integrated SPR from a per-recruit cohort tracked across every region
#' and season under movement, with one scalar \eqn{F_x} applied across regions and
#' scaled by the region's fleet fractions and selectivity. Returns the squared
#' penalty \eqn{100 (SPR - SPR_x)^2} for the optimizer.
#'
#' Single and multi-population models are both covered. When \code{n_pop > 1},
#' effective SSB at each population's natal region accumulates straying
#' contributions from the others, and when \code{n_seas = 1} as well,
#' \code{sgl_seas_spawning_movement} redistributes fish to the natal grounds
...
#' @param pars Named list of RTMB parameters, holding \code{log_F_x}, the
#'   log-scale trial fishing mortality.
#' @param data Named list of RTMB data: the dimensions \code{n_pop},
#'   \code{n_regions}, \code{n_ages} and \code{n_seas}; \code{seasdur}
#'   \code{[n_seas]}; \code{spawn_seas} and \code{t_spawn}; \code{F_fract_flt}
#'   \code{[n_regions, n_seas, n_fish_fleets]}, the fleet F fractions by region;
#'   \code{fish_sel} and \code{ret_sel} \code{[n_pop, n_regions, n_seas, n_ages,
#'   n_fish_fleets]}, the female selectivity and the retained fraction of it;
#'   \code{dmr} \code{[n_regions, n_seas, n_fish_fleets]}, the fraction of discards
#'   that die; \code{natmort} \code{[n_pop, n_regions, n_ages]}; \code{WAA} and
#'   \code{MatAA} \code{[n_pop, n_regions, n_seas, n_ages]}; \code{Movement}
#'   \code{[n_pop, n_regions, n_regions, n_seas, n_ages]} and
#'   \code{sgl_seas_spawning_movement} \code{[n_pop, n_regions, n_regions,
#'   n_ages]}; \code{do_recruits_move}; \code{rec_region_prop} and
#'   \code{sex_ratio_f} \code{[n_pop, n_regions]}; \code{rec_seas_prop}
#'   \code{[n_pop, n_seas]}; \code{stray_rate} and \code{natal_region}
#'   \code{[n_pop]}; \code{n_pop_in_region} \code{[n_regions]}; and the target
#'   \code{SPR_x}.
#'
#' @return Numeric scalar, the squared penalty \eqn{(SPR - SPR_x)^2}.
#'
#' @keywords internal
#' @import RTMB
global_SPR <- function(pars,
  ...
  if(spawn_seas > 1) {
    for(p in 1:n_pop) {
      for (seas in 1:(spawn_seas - 1)) {
        ...
        M_seas_plus = natmort[p,,seas,n_ages] * seasdur[seas]
  ...
  # accumulate effective SSB at each population's natal region across all source populations,
  # dividing by n_pop_in_region to preserve mass balance when populations share a natal region
  for(p2 in 1:n_pop) {
    r <- natal_region[p2]
    for(p in 1:n_pop) {
      sc <- if(p == p2) 1 else stray_rate[p] / n_pop_in_region[r]
