...
| ObsCatchAA, ObsDiscardAA, ObsFishIdxAA | Age-disaggregated retained catch, discards and fishery index, each dimensioned by n_regions, n_years, n_seas, n_ages, n_sexes, n_fish_fleets. A fleet fits these or the aggregated form with compositions, never both. The sex margin is required whatever the fleet reports: a stream summed over sexes carries its observation in sex slot one, and an array a dimension short is refused |
| UseCatchAA, UseDiscardAA, UseFishIdxAA | Arrays shaped like their observations, 1 where an observation is fit. A cell not fit for catch at age is also not fished, the way UseCatch governs closures for the aggregated stream |
| ObsCatchAA_SE, ObsDiscardAA_SE, ObsFishIdxAA_SE | Reported standard errors shaped like their observations, read only when the stream’s sigma_form asks for them |
| CatchAA_Type, DiscardAA_Type, FishIdxAA_Type | Vectors dimensioned by n_fish_fleets naming the margins each fleet reports separately. 0: summed over regions and sexes, 1: split by region and summed over sexes (default), 2: summed over regions and split by sex, 3: split by both |
| CatchAA_LikeType, DiscardAA_LikeType, FishIdxAA_LikeType | Vectors dimensioned by n_fish_fleets. 0: Lognormal (default), 1: Normal on the arithmetic scale |
| CatchAA_sigma_form, DiscardAA_sigma_form, FishIdxAA_sigma_form | Vectors dimensioned by n_fish_fleets naming where the observation error comes from. 0: the estimated parameter alone (default), 1: the reported standard errors alone, 2: additive, 3: in quadrature |
| AgeObsCorr_catch, AgeObsCorr_discard, AgeObsCorr_fish_idx | Vectors dimensioned by n_fish_fleets giving the correlation across ages within a cell. 0: iid, 1: AR(1) in age distance, 2: unstructured, 3: separable AR(1) over ages and years. Each has a \_pop counterpart, since the population-specific streams carry their own |
...
| SrvIdxAA_Type, SrvIdxAA_LikeType, SrvIdxAA_sigma_form | Vectors dimensioned by n_srv_fleets, following the same codes as their fishery counterparts |
