...
| init_devs_pen_use | Array dimensioned by n_pop, n_regions, n_ages - 1, n_sexes of 0/1 flags naming which cells of `ln_InitDevs` the initial age penalty covers, so a curve shared across sexes (`InitDevs_sex_spec = "est_shared_s"`) is penalized once and sex-specific curves (`"est_all"`) each are |
...
| AgeingError | Ageing error matrix dimensioned by n_years, n_modelled_ages, n_observed_ages, where rows across modeled ages sum to 1. Maps the model’s ages onto the observed ages that the age compositions and the at-age data sources are recorded on |
...
| Fixed_natmort | Natural mortality array dimensioned by n_pop, n_regions, n_years, n_ages, n_sexes, or by n_pop, n_regions, n_years, n_seas, n_ages, n_sexes when the rate differs between seasons. Values are instantaneous rates per year in both forms, and the array without a season dimension is expanded across seasons |
| M_blocks | Array that specifies the natural mortality blocking structure, dimensioned by n_pop, n_regions, n_years, n_seas, n_ages, n_sexes. Unique integer values denote distinct natural mortality parameter blocks |
...
| cont_vary_movement | Character string giving the structure of continuous movement deviations. “none”, or “iid\_” followed by the dims they vary over, any of p (population), y (year), seas (season), a (age), s (sex), in any combination. A dim left out shares one deviation across it |
| map_move_devs | Array dimensioned by n_regions, n_regions - 1 (1 under CTMC movement, whose deviations sit on a region’s preference), n_years, n_ages, and n_sexes specifying which movement parameters are shared and mapped off |
...
| ctmc_diffusion_bounds | How the generator is kept Metzler: `"none"` (0), `"softplus"` (1), or `"upwind"` (2), the discontinuous Galerkin flux, which has diffusion whole and reads no eps |
...
| ObsCatchAA, ObsDiscardAA | Age-disaggregated retained catch and discards, each dimensioned by n_regions, n_years, n_seas, n_observed_ages, n_sexes, n_fish_fleets, the observed ages being the columns of AgeingError_fish, through which the predicted values at each model age are read. A fleet fits these or the aggregated form with compositions, never both. The sex dim is required whatever the fleet reports: a data source summed over sexes has its observation in sex slot one, and an array a dimension short is refused |
| UseCatchAA, UseDiscardAA | Arrays shaped like their observations, 1 where an observation is fit. A cell not fit for catch at age is also not fished, the way UseCatch governs closures for the aggregated data source |
| ObsCatchAA_SE, ObsDiscardAA_SE | Reported standard errors shaped like their observations, read only when the data source’s sigma_form asks for them |
| CatchAA_Type, DiscardAA_Type | Matrices dimensioned by n_years, n_fish_fleets naming the dims each fleet reports separately, which may change between years. 0: summed over regions and sexes, 1: split by region and summed over sexes (default), 2: summed over regions and split by sex, 3: split by both |
...
| AgeObsCorr_catch, AgeObsCorr_discard | Vectors dimensioned by n_fish_fleets giving the correlation across ages within a cell. 0: iid, 1: AR(1) in age distance, 2: unstructured, 3: separable AR(1) over ages and years. Each has a \_pop counterpart, since the population-specific data sources have their own |
...
| ObsSrvIdxAA | Age-disaggregated survey index dimensioned by n_regions, n_years, n_seas, n_observed_ages, n_sexes, n_srv_fleets, read through AgeingError_srv. A fleet fits this or the aggregated index, never both. Same shape rules as ObsCatchAA |
...
| srv_q_model | Vector dimensioned by n_srv_fleets giving the process error on annual survey catchability deviations. 1: none, 2: iid, 3: random walk, 4: ar1, 5: dsem |
| srv_q_rw_init_sigma | Standard deviation of the first year of a survey catchability random walk. NA starts the walk at zero under its own sigma |
| map_ln_srv_q_devs | Array dimensioned by n_regions, n_years, n_srv_fleets marking which survey catchability deviations the penalty reads |
...
| fish_q_type | Vector dimensioned by n_fish_fleets specifying how fishery catchability is obtained. Same codes and behavior as srv_q_type. 0: Estimated as exp(ln_fish_q) (default), 1: Solved analytically as the ratio of mean observed to mean predicted index, 2: Solved analytically on the log scale as exp(mean(log(obs) - log(pred))). Analytic fleets have their ln_fish_q fixed automatically, ignore block structure, and cannot have catchability deviations or priors |
| fish_q_model | Vector dimensioned by n_fish_fleets giving the process error on annual fishery catchability deviations. 1: none, 2: iid, 3: random walk, 4: ar1, 5: dsem |
| fish_q_rw_init_sigma | Standard deviation of the first year of a fishery catchability random walk. NA starts the walk at zero under its own sigma |
| map_ln_fish_q_devs | Array dimensioned by n_regions, n_years, n_fish_fleets marking which fishery catchability deviations the penalty reads |
...
| map_ln_fishsel_bin_devs | Array mirroring the ln_fishsel_bin_devs factor map: an estimation index where a deviation is estimated, NA where fixed. Only overridden bins have estimated deviations |
