...
has its own deviations $`\epsilon^{\text{init}}_{a,s}`$. Those
...
sharing a curve is how that is written: fleet 1 holds the index at
$`t^{\text{srv}} = (7 - 1)/12`$, fleet 2 holds the compositions at
...
The assessment has two weak penalties on fishing mortality, one pulling
annual $`F`$ toward 0.2 and one forcing the mean deviation to zero.
`ln_F_mean_spec = "fix"` does the same thing by fixing the mean, so the
deviations have all of $`\log F`$. Hoervrt, yhr first has no counterpart
in `SPoRC` is left out.

## Fishery compositions

Compositions are joint across sexes, females then males, so each year’s
observation holds the sex ratio as well as the age structure.
...
which holds the sex ratio. The input sample size goes in the first sex’s
slot, since the joint composition is one observation rather than two.
There is no fishery index, so `fish_idx_type = "none"` with the use
...
$`\sigma = 0.2/\sqrt{42}`$ holds the same information, which is what
...
Every weight is one. The assessment’s own weights are kept as standard
deviations where they belong instead: the catch weight of 300 became
$`\sigma_{C}`$, and the composition weights of one are the input sample
sizes.
...
# starts at the mean of its cells, so both fleets hold the value.
for(sf in 1:n_srv) {
...
$`F`$ scale, which is a numerical stabilizer left over from the early
estimation phases rather than a statement about fishing mortality, so
`SPoRC` does not have it.
