...
percent, which the Beverton-Holt then propagates into recent
recruitment. `SPoRC` forms the first year’s spawning biomass from the
initial numbers and every later year’s from the survivors, so a maturity
at age 0 that is the fecundity ratio in $`y_{1}`$ and zero afterwards is
exact.
...
`SPoRC` currently has no parameter for it, so it is kept at the
...
on abundance. `SPoRC` holds this as `srv_idx_type = "recdev"`, a survey
fleet that observes year class strength directly and reads no part of
the population.

Two fleets have two composition data sources. The trawl fishery and the
West Coast groundfish bottom trawl survey each have sexed and unsexed
...
spawning biomass is numbers times fecundity, with the age 0 cell with
its value in the equilibrium year alone.
...
leave the deviations with all of $`\log F`$. Fixing the mean matters:
with the deviations unpenalized the mean and their average are exactly
redundant, and fixing one of them is what keeps that ridge from going
off the rails.
...
can’t have two sets of composition data.
...
`FishAgeComps_Type` is where the two composition data sources show up.
The three gear fleets have sexed compositions, which is `spltRjntS`,
females and males in one vector normalized together. The three discard
fleets and the trawl companion have compositions with the sexes pooled,
which is `agg`.
...
hold the extra standard deviation added to them.
...
composition companion of the bottom trawl survey has no index, so its
catchability stays out of the parameter vector.
...
`fish_sel_sex_offset = "par_apical"` is what holds the male curve. The
...
likelihood enforces; and the extra survey standard deviation is kept at
...
do not have that source of uncertainty.
