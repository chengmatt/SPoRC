...
aggregate_regions <- function(d) {
  ...
  pool <- function(C) {
    ...
    for(y in 1:n_yrs) {
      num <- colSums(C[, y, 1, , 1, 1] * w[, y])
      out[1, y, 1, , 1, 1] <- num / sum(num)
    }
    out
  }
  a$ObsSrvAgeComps <- pool(d$ObsSrvAgeComps)
  a$ObsFishAgeComps <- pool(d$ObsFishAgeComps)
  a$UseSrvAgeComps <- array(1, dim = c(1, n_yrs, 1, 1))
  a$UseFishAgeComps <- a$UseSrvAgeComps
