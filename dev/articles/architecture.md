...
|----|----|----|
| `test-setup_*` | 15 | Input building: the `map` factor builders and the `Setup_Mod_*` validation |
...
| `test-regression_*` | 14 | End to end fits pinning `obj$rep` and `nll` for known configurations |

That is 94 test files in total.
