...
| `trans_rho_catch` | `n_sexes × n_fish_fleets` | Unconstrained AR(1) correlation across ages for catch at age. Transformed to `(-1, 1)` by [`rho_trans()`](https://chengmatt.github.io/SPoRC/dev/reference/rho_trans.md). One per fleet shared across sexes unless `rho_catch_key` says otherwise, and mapped to `NA` unless `AgeObsCorr_catch` is `"1dar1"` or `"2dar1"` | 0 |
| `trans_rho_catch_year` | `n_sexes × n_fish_fleets` | Correlation across years, read only under `AgeObsCorr_catch = "2dar1"` | 0 |
| `trans_rho_catch_us` | `n_pairs × n_sexes × n_fish_fleets` | Unconstrained entries of the unstructured across-age correlation, `n_pairs = n_ages(n_ages - 1)/2`. Read only under `AgeObsCorr_catch = "us"` | 0 |
...
| `trans_rho_srv_idx`, `trans_rho_srv_idx_year`, `trans_rho_srv_idx_us` | `n_sexes × n_srv_fleets` (and `n_pairs × …`) | As above, for the survey index at age | 0 |
| `trans_rho_catch_pop`, `trans_rho_discard_pop`, `trans_rho_fish_idx_pop`, `trans_rho_srv_idx_pop` | as above | Population-specific counterparts, with their own `_year` and `_us` forms. The `_pop` streams carry their own correlation rather than borrowing the aggregated one | 0 |
