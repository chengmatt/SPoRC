...
Note that `fish_fixed_sel_pars` holds parameters on the scale of the
...
| `ln_fish_q_devs` | `n_regions × n_years × n_fish_fleets` | Log-scale annual deviations about the mean fishery catchability, under `fish_q_model` | 0 |
| `ln_sigma_fish_q` | `n_regions × n_fish_fleets` | Log-scale standard deviation of the fishery catchability deviations | log(0.1) |
| `fish_q_rho` | `n_regions × n_fish_fleets` | Unconstrained ar1 correlation of the fishery catchability deviations, read only under `fish_q_model = "ar1"` | 0 |
...
| `ln_srvsel_bin_devs` | `n_regions × (n_years + n_proj_yrs_devs) × n_bins × n_sexes × n_srv_fleets` | Log-scale bin-override selectivity deviations (see `ln_fishsel_bin_devs` in [Fishery Selectivity](#fishery-selectivity)) | 0 |
| `ln_srvsel_sex_scale` | `n_regions × n_blocks × n_sexes × n_srv_fleets` | Log-scale curve multiplier for sexes beyond the first on survey selectivity (see `ln_fishsel_sex_scale` in [Fishery Selectivity](#fishery-selectivity); `srv_sel_sex_offset`) | 0 |
...
| `ln_srv_q_devs` | `n_regions × n_years × n_srv_fleets` | Log-scale annual deviations about the mean survey catchability, under `srv_q_model` | 0 |
| `ln_sigma_srv_q` | `n_regions × n_srv_fleets` | Log-scale standard deviation of the survey catchability deviations | log(0.1) |
| `srv_q_rho` | `n_regions × n_srv_fleets` | Unconstrained ar1 correlation of the survey catchability deviations, read only under `srv_q_model = "ar1"` | 0 |
...
| `NAA_season_corr_pars` | `n_pop × n_k(n_k - 1)/2 × n_sexes` | Unconstrained parameters for an unstructured correlation across the $`n_k`$ active seasons, shared according to `NAA_re_season_spec` | 0 |
...
was called. The grid has one column per series, covariates first, then
the linked deviation series.

| Parameter | Dimension | Description | Default |
|----|----|----|----|
| `dsem_beta` | `n_paths` | Path coefficients, one per named path or covariance arrow, in the order the arrows name them (`data$dsem_model$beta_names`). A fixed arrow (`NA` name, value in the fourth field) has no entry here. | 0, or the arrow’s start value |
| `ln_dsem_sd` | `n_sd_lines` | Log innovation standard deviations, one per sd line (`data$dsem_model$ln_sd_names`). | log(1), or the arrow’s start value |
| `dsem_mu` | `n_series` | Series means. Estimated for the covariates `dsem_mu_spec` names, kept at the observed mean otherwise, and always zero for a deviation series. | observed mean, 0 |
| `dsem_x` | `n_grid_years × n_series` | The dsem grid. Known covariate cells and linked deviation cells are mapped off (the objective writes the deviations in); missing covariate years, `"normal"` covariates and the rows a linked series does not reach (the forecast) are estimated. | the series means |
| `logit_dsem_tweedie_p` | `n_covariates` | Tweedie power as `1 + plogis(.)`, estimated for `dsem_family = "tweedie"` only. |  |
| `ln_dsem_obs_sd` | `n_covariates` | Log spread of each covariate’s observations on its family’s scale: sd (`"normal"`), CV (`"gamma"`), sd of the log (`"lognormal"`), dispersion (`"tweedie"`); mapped off for `"fixed"`. | log(sd / 2) |
| `dsem_delta0` | `n_series` | First-year offset of each series, estimated for those `dsem_delta0_spec` names. | 0 |
