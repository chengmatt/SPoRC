...
| Object | Dimensions |
|----|----|
| `CatchAA_nLL` | region, year, season, observed age, sex, fishery fleet |
| `DiscardAA_nLL` | region, year, season, observed age, sex, fishery fleet |
| `SrvIdxAA_nLL` | region, year, season, observed age, sex, survey fleet |
...
The age dim is the observed ages, the columns of the fleet’s ageing
error matrix, which are the model ages under the default identity
matrix. A data source summed over regions or sexes writes into slot one
of those dims and leaves the rest at zero, so the array’s shape does not
tell you what the fleet reported; its `Type` does. A cell whose ages are
...
`ln_sigmaDAA`, and `ln_sigmaSrvIdxAA`, each over observed age, sex and
fleet. The age shape of catchability is not among them: an index fit age
by age has it in selectivity, through the `"nonparfree"` form.
