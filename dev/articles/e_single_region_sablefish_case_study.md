...
`sgl_rg_sable_data`. The companion script that produced the figures
below is `dev/make_sporc_obj_figs/make_sgl_rg_sablefish_bridge_figs.R`,
...
exactly that sum of squares and the assessment’s weights carry over
...
Data sources carry a $`\lambda`$ weight that multiplies the whole
