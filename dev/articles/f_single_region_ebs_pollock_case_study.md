...
5.  Initial age structure is derived by assuming a geometric series
    (`init_age_strc = 2`; the alternative is iterating age structure to
...
``` r
  ...
  input_list <- Setup_Mod_Rec(
    ...
    ln_sigmaR = array(log(c(1, 1)), dim = c(2, input_list$data$n_pop, input_list$data$n_regions)),
...
`n_pop`, `n_regions`, `n_years`, `n_seas`, `n_ages`, `n_sexes`. In the
case of Bering Sea pollock, natural mortality is age-specific and fixed.
Thus, a natural mortality array is constructed and passed into the
function, which is also dimensioned by `n_pop`, `n_regions`, `n_years`,
`n_ages`, `n_sexes`.
...
necessary is updating the `input_list` and setting the
`use_conv_fish_tagging` argument to a vector of 0s (one per fishery
fleet) to indicate that no tagging data are used.
...
`n_years`, `n_seas`, and `n_fish_fleets`. Additional model options
specified for this case study include whether fishing mortality
penalties are utilized to help estimate deviations `Use_F_pen = 1`, and
the `ln_sigmaC` values to fix at.
...
`ObsFishIdx_SE`), which are dimensioned by `n_regions`, `n_years`,
`n_seas`, `n_fish_fleets`. Additionally, we will supply both fishery age
composition data and length composition data. Both `ObsFishAgeComps` and
`ObsFishLenComps` are dimensioned by `n_regions`, `n_years`, `n_seas`,
`n_bins (n_ages | n_lens)`, `n_sexes`, `n_fish_fleets`. Because the
Bering Sea pollock model does not fit to length composition data,
`ObsFishLenComps` is specified as `NA` and `UseFishLenComps` is
...
data, and fishery age compositions are aggregated across regions and
...
Here, we will supply arrays of survey indices and their associated
standard errors (`ObsSrvIdx` and `ObsSrvIdx_SE`), which are dimensioned
by `n_regions`, `n_years`, `n_seas`, `n_srv_fleets`. In this case, we
are modelling three survey fleets, where fleet 1 is the bottom trawl
survey, fleet 2 is the acoustic trawl survey, and fleet 3 is the
acoustic vessel of opportunity survey. We will also input both survey
age composition data and length composition data. Both `ObsSrvAgeComps`
and `ObsSrvLenComps` are dimensioned by `n_regions`, `n_years`,
`n_seas`, `n_bins (n_ages | n_lens)`, `n_sexes`, `n_srv_fleets`. All
three survey indices are biomass-based and multinomial likelihoods are
used to fit to age-compositions for fleets 1 and 2. However, because the
acoustic vessel of opportunity survey does not have composition data,
these are not fit to (and hence specified as none).
...
pollock model, with the `2dar1` option specified. Here, we will estimate
selectivity deviations in a penalized framework and fix variance values
a priori `fishsel_pe_pars_spec = "fix"`. Moreover, we will estimate
...
``` r
  ...
  input_list <- Setup_Mod_Fishsel_and_Q(
    ...
    corr_opt_semipar = "corr_zero_y_b", # making sure 2d correlations are 0, collapses to a simple iid case
    ...
    # whether to estimate all fixed effects for fishery selectivity
    fish_fixed_sel_pars_spec = c("est_all"),
    # whether to estimate all fixed effects for fishery catchability
...
``` r
 ...
 # Setup survey selectivity and catchability
  input_list <- Setup_Mod_Srvsel_and_Q(
    ...
    cont_tv_srv_sel = c("2dar1_Fleet_1", "2dar1_Fleet_2", "2dar1_Fleet_3"),
    ...
    corr_opt_semipar = c("corr_zero_y_b", "corr_zero_y_b", "corr_zero_y_b"), # setting correlations at 0, so 2dar1 collapses to simple iid semi-parametric devs
    ...
    # whether to estimate all fixed effects for survey selectivity
    srv_fixed_sel_pars_spec = c("est_all", "est_all", "est_shared_f_2"), # sharing fleet 3 and 2 parameters together
    # whether to estimate all fixed effects for survey catchability
...
selectivity variances for a logistic function are fixed at 0.075.
Lastly, the acoustic trawl and vessel of opportunity surveys will have a
...
parameters$fishsel_pe_pars[,4,,] <- log(0.3) # fishery selex variance
parameters$srvsel_pe_pars[,1:2,,1] <- log(0.3) # survey BTS - a50 and delta variance
parameters$srvsel_pe_pars[,4,,2] <- log(0.3) # survey ATS and ato variance
...
biomass and compare to the 2024 pollock model. Since `Rec` and `SSB` are
both dimensioned `n_pop × n_regions × n_years`, we rename the melted
...
rec_series <- reshape2::melt((ebswp_rtmb_model$rep$Rec)) %>%
  mutate(se = ebswp_rtmb_model$sd_rep$sd[names(ebswp_rtmb_model$sd_rep$value) == 'log_Rec'] * as.vector(ebswp_rtmb_model$rep$Rec))
rec_series$Par <- "Recruitment"

# Get SSB time-series
ssb_series <- reshape2::melt((ebswp_rtmb_model$rep$SSB)) %>%
  mutate(se = ebswp_rtmb_model$sd_rep$sd[names(ebswp_rtmb_model$sd_rep$value) == 'log_SSB'] * as.vector(ebswp_rtmb_model$rep$SSB))
