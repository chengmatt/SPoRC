...
serving as a burn-in period. A total of 100 simulations will be
...
setup_em <- function(sim_env, y, sim) {
  ...
  input_list <- Setup_Mod_Biologicals(
    ...
    Fixed_natmort = array(0.07, dim = c(input_list$data$n_pop, input_list$data$n_regions, length(input_list$data$years),
  ...
  for (y in 1:sim_env$n_yrs) {
    ...
    if(y >= sim_env$feedback_start_yr) {
      ...
      if(y %in% assessment_years) {
        ...
        # set data to 0 (assuming assessments are not conducted if no new data are available)
