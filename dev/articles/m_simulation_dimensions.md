...
| `NAA_bef` | `n_pop × n_regions × n_years × n_seas × n_ages × n_sexes × n_sims` | Numbers-at-age at the start of the season, before movement is applied |
| `NAA_aft` | `n_pop × n_regions × n_years × n_seas × n_ages × n_sexes × n_sims` | Numbers-at-age after movement is applied. Under `move_timing = 0` this is post-movement but pre-mortality; under `move_timing` 1 and 2 movement is resolved at the end of the season, so it is the post-season state (movement and mortality together) |
