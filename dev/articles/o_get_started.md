...
time-varying selectivity is specified, and selectivity is modeled as
...
time-vaying selectivity is specified, and selectivity is modeled as
...
residuals with
[`SPoRC::plot_resids`](https://chengmatt.github.io/SPoRC/dev/reference/plot_resids.md),
...
fishages <- get_osa(obs_mat = comp_prop$Obs_FishAge_mat, # observed fishery age compositions
                    ...
                    N = francis_data$ISS_FishAgeComps[1,,1,1,1] * # input sample size, at the full year length
                      unique(francis_data$Wt_FishAgeComps[1,,1,1,1]), # francis weight
                    years = which(francis_data$UseFishAgeComps[1,,1,1] == 1), # years with fishery ages
                    seas = 1, # season
                    fleet = 1, # fleet
                    bins = 4:30, # age bins
...
fishlens <- get_osa(obs_mat = comp_prop$Obs_FishLen_mat, # observed fishery length compositions
                    ...
                    N = francis_data$ISS_FishLenComps[1,,1,1,1] * # input sample size, at the full year length
                      unique(francis_data$Wt_FishLenComps[1,,1,1,1]), # francis weight
                    years = which(francis_data$UseFishLenComps[1,,1,1] == 1), # years with fishery lengths
                    seas = 1, # season
                    fleet = 1, # fleet
                    bins = 21:52, # age bins
                    comp_type = 0, # composition type (age-specific)
...
srvages <- get_osa(obs_mat = comp_prop$Obs_SrvAge_mat, # observed survey age compositions
                   ...
                   N = francis_data$ISS_SrvAgeComps[1,,1,1,1] * # input sample size, at the full year length
                     unique(francis_data$Wt_SrvAgeComps[1,,1,1,1]), # francis weight
                   years = which(francis_data$UseSrvAgeComps[1,,1,1] == 1), # years with survey ages
                   seas = 1, # season
                   fleet = 1, # fleet
                   bins = 4:30, # age bins
                   comp_type = 0, # composition type (age-specific)
...
`adnuts`) to conduct MCMC analysis, which estimates parameter
uncertainty and samples the full joint posterior, showing parameter
correlations, skewed distributions, or potential multimodality that MLE
may miss. Comparing MCMC results with maximum likelihood estimates
further helps assess consistency in point estimates and standard errors.
In particular, we can utilize the native functionality built into
`adnuts`, which works directly with TMB and RTMB models, which will be
demonstrated in the following sections. First, let us install and load
...
scenarios <- reshape2::melt(all_scenarios_catch) %>%
  mutate(Year = Var3 + 2023,
         Scenario = case_when(
           ...
           Var7 == 6 ~ "S6: FOFL",
           Var7 == 7 ~ "S7: FABC -> FOFL",
           TRUE ~ paste("Scenario", Var7)
         ),
         Type = "Projection") %>%
  rename(Region = Var2, Simulation = Var6, Catch = value, Fleet = Var5)
