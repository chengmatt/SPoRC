...
[`SPoRC::simulation_data_to_SPoRC()`](https://chengmatt.github.io/SPoRC/dev/reference/simulation_data_to_SPoRC.md)
...
R0_input <- array(0, dim = c(n_pop, n_regions, n_years + closed_loop_yrs))
...
h_input <- array(0, dim = c(n_pop, n_regions, n_years + closed_loop_yrs))
...
remained stable, with lower harvest for capped MPs (**f40_hybrid**) and
...
`crash` scenario, differences became larger: MPs with higher biomass
reference points (e.g., **f40_hybrid**) reduced catch early, maintaining
higher SSB (but at the cost of increased catch variability), while
**f40_const** allowed higher catch during low recruitment but lower
catch after recovery. Overall, **f40_hybrid** offered balanced
trade-offs, supporting intermediate catch, improved spawning stock
biomass, and reduced catch variability.
