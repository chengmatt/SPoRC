...
1.  **Unstructured Markov**: Discrete movement matrices with optional
    blocking structures across population, age, year, season, and sex
    dimensions
2.  **Continuous-Time Markov Chain (CTMC)**: CTMC-based movement with
    diffusion and preference parameters estimated using formula-based
    approaches

The following demonstrations use the three region sablefish dataset as a
basis (`three_rg_sable_data`). The initial setup establishes the general
model dimensions. Note that `n_pop = 1` and `n_seas = 1` are specified
here, but movement can be configured for multi-population and
multi-season models in an analogous fashion.
...
between regions. This results in `n_regions × (n_regions - 1)`
...
in `n_regions × (n_regions - 1) × 2` parameters estimated.
...
are specified, resulting in `n_regions × (n_regions - 1) × 5` parameters
estimated.
...
`n_regions × (n_regions - 1) × n_sexes` parameters estimated.
...
`n_regions × (n_regions - 1) × 2 × 5 × n_sexes` movement parameters
being estimated. However, this parameterization is likely excessive and
may lead to an unstable model solution. Note that this model uses 1
population and 1 season, but population-specific and season-specific
movement rates can be specified in an analogous fashion using
`Movement_popblk_spec` and `Movement_seasblk_spec`.
...
transitions among regions.

To implement a CTMC-based movement model, an adjacency matrix must first
be defined to specify which regions are connected. In the example below,
all regions are assumed to be connected, permitting individuals to move
among any spatial strata within a given period. An accompanying data
frame is then created to define the covariates associated with movement.
...
ctmc_data <- expand.grid(
  ...
  years   = seq_along(three_rg_sable_data$years),
  seas    = 1,
  ages    = seq_along(three_rg_sable_data$ages),
...
regions. This formulation results in the estimation of `n_regions`
parameters, providing a more parsimonious alternative to the
unstructured Markov approach that estimates
`n_regions × (n_regions - 1)` parameters. Movement among adjacent areas
is determined by the defined adjacency matrix, and no directional
preference is specified in this case.
...
`n_regions` additional parameters describing age-specific movement
preferences.
...
estimated, along with `n_regions × 4` parameters describing age-specific
...
region, age, year, and sex. For multi-population models, `factor(pop)`
can be added to the formula in an analogous fashion. This specification
results in the estimation of `n_regions × 4 × 6 × n_sexes` preference
parameters.
...
origin-destination pair, so deviations are estimated for destination
regions only (i.e., `n_regions - 1`), the source region having no
deviation term. Under CTMC movement a deviation is added to a region’s
own habitat preference, so there is one per region and `move_devs` holds
a single destination level. That one deviation raises the rate into its
region from every neighbor and lowers the rate out of it, which is a
year in which the region was simply a better place to be, rather than a
year in which one boundary happened to be busier. Only differences in
preference reach the generator, so adding the same deviation to every
region leaves movement unchanged and the level of the field is held down
by the process error penalty alone.

Note that because preference enters the generator additively, a
deviation larger than an edge’s diffusion rate can potentially drive
that rate negative, and with `ctmc_diffusion_bounds = "none"` the
resulting movement fractions end up departing from the `[0, 1]` while
still summing to one (so abundance is not conserved). It is recommended
to use a bounded form (`"upwind"` or `"softplus"`) whenever CTMC
