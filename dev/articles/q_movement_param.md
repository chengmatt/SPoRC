...
specified to be shared across populations, source regions, seasons,
ages, and sexes (`Movement_cont_pe_pars_spec = 'est_shared'`).
Additional options for `cont_vary_movement` and
`Movement_cont_pe_pars_spec` are described in the function documentation
...
Note that deviations are only estimated for destination regions (i.e.,
`n_regions - 1`), as no deviation term is defined for the source region.
