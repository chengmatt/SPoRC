# Purpose: Bridge the 2024 BSAI northern rock sole assessment (fm.tpl, Model 24.2) to SPoRC and render its figures
...
# likelihood crosswalk. SPoRC writes each component as a proper density while the assessment drops
# normalizing constants, so each Gaussian block is compared net of exactly those constants
...
ggplot2::ggsave(
  here("vignettes", "figures", "ac_bsai_nrs_ts_comparison.png"),
  p_ts,
  width = 17,
  height = 9,
  dpi = 200
...
ggplot2::ggsave(
  here("vignettes", "figures", "ac_bsai_nrs_sel_comparison.png"),
  bridge_sel_figure(sel_df, nrow = 2),
  width = 15,
  height = 9,
  dpi = 200
)
