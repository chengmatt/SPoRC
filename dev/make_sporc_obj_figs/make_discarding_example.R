# Purpose: To create an example of discarding dynamics
...
sim_list <- Setup_Sim_Fishing(sim_list = sim_list, # update simulate list
                              ...
                              fish_sel_input = replicate(
                                n = sim_list$n_sims,
                                array(rep(1 / (1 + exp(-3 * ((1:sim_list$n_ages) - 2))), each = sim_list$n_yrs),
                              ...
                              ret_sel_input = replicate(
                                n = sim_list$n_sims,
                                array(rep(0.5 / (1 + exp(-3 * ((1:sim_list$n_ages) - 5))), each = sim_list$n_yrs),
                                      dim = c(sim_list$n_pop, sim_list$n_regions, sim_list$n_yrs, sim_list$n_seas, sim_list$n_ages,
                                              sim_list$n_sexes, sim_list$n_fish_fleets))
                              ),
                              dmr_input = array(0.5, dim = c(sim_list$n_regions, sim_list$n_yrs, sim_list$n_seas, sim_list$n_fish_fleets, sim_list$n_sims))
)
...
setup_em <- function(sim_obj, sim) {
  ...
  input_list <- Setup_Mod_Fishsel_and_Q(
    ...
    use_fixed_ret_sel       = 0,
    fish_sel_nonpar_est_bins = list(list(list(1, 2, 3, 4, 5:10))),
    ret_sel_nonpar_est_bins  = list(list(list(1, 2, 3, 4, 5:10)))
  )
  ...
  input_list <- Setup_Mod_Srvsel_and_Q(
    ...
    srv_q_spec              = "est_all",
    srv_sel_nonpar_est_bins = list(list(list(1, 2, 3, 4:10)))
  )
...
ggsave(
  filename = here("vignettes", "figures", "s_ssb_re.png"), width = 8, height = 8,
