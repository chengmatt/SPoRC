...
setup_em <- function(sim_env, y, sim) {
  ...
  input_list <- Setup_Mod_Dim(years = 1:y, # vector of years
                              ...
                              n_seas = sim_env$n_seas, # number of seasons
                              verbose = F
  )
  ...
  input_list <- Setup_Mod_Catch_and_F(
    ...
    ln_sigmaF = array(log(sqrt(1/2)), dim = c(input_list$data$n_regions, input_list$data$n_seas,
    ...
    if(y >= sim_env$feedback_start_yr) {
      ...
      if(y %in% assessment_years) {
        ...
        asmt_data <- set_data_indicator_unused(data = asmt_data,
                                               ...
                                               what = c("FishIdx", "FishAgeComps", "SrvIdx",
                                                        "SrvAgeComps", "FishLenComps", "SrvLenComps"))
      ...
      if(y %in% assessment_years) {
        ...
        tmp_terminal_NAA <- array(obj$rep$NAA[,y,,,], dim = c(asmt_data$n_regions, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes)) # terminal numbers at age
        tmp_terminal_NAA0 <- array(obj$rep$NAA0[,y,,,], dim = c(asmt_data$n_regions, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes)) # terminal numbers at age
        tmp_WAA <- array(rep(asmt_data$WAA[,y,,,], each = proj_opt$n_proj_yrs), dim = c(asmt_data$n_regions, proj_opt$n_proj_yrs, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes)) # weight at age
        tmp_WAA_fish <- array(rep(asmt_data$WAA_fish[,y,,,,], each = proj_opt$n_proj_yrs), dim = c(asmt_data$n_regions, proj_opt$n_proj_yrs, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes, asmt_data$n_fish_fleets)) # weight at age fishery
        tmp_MatAA <- array(rep(asmt_data$MatAA[,y,,,], each = proj_opt$n_proj_yrs), dim = c(asmt_data$n_regions, proj_opt$n_proj_yrs, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes)) # maturity at age
        ...
        tmp_Movement <- array(dim = c(asmt_data$n_regions, asmt_data$n_regions, proj_opt$n_proj_yrs, asmt_data$n_seas, length(asmt_data$ages), asmt_data$n_sexes))
      ...
      if(y < sim_env$n_yrs) {
        ...
        tmp_f <- mapply(function(r, f) { # do bisection to go from region and fleet specific catch to region and fleet specific F rates
          catch_to_F_singlefleet(
            ...
            catch = tmp_TAC[r, tac_year_index, 1, f], # catch values to use
            NAA = sim_env$NAA[r, y+1, 1, , , sim], # numbers at age in simulation (truth)
            WAA = sim_env$WAA[r, y+1, 1, , , sim], # weight-at-age in simulation (truth)
        ...
        sim_env$Fmort[,y+1,1,,sim] <- array(tmp_f, dim = c(sim_env$n_regions, sim_env$n_fish_fleets)) # assign bisection values back into simulation
      } # end if
