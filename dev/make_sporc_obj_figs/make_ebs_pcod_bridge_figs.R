# Purpose: Bridge the 2024 EBS Pacific cod assessment (SS3 Model 24.1) to SPoRC and render its figures
# Creator: Matthew LH. Cheng
# Date Created: 8/23/26
#
# built through tests/testthat/helper-bridge_ebs_pcod.R, which is the readable specification
...
# fit_model returns the RTMB object without an sdreport, so it is taken here. the analytical
# Hessian is passed rather than letting sdreport call optimHess, which reports a false non-pd
...
# at the estimate and refitted. SPoRC's single sex spawning biomass is the female share, so it
# is doubled onto the assessment's own definition
...
# the first panel has no interval: the model is evaluated at the assessment's estimate rather than
# fitted, so the curvature there is not an uncertainty statement about anything SPoRC estimated
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_ts_comparison.png"),
  p_ts,
  width = 15,
  height = 10,
  dpi = 170
...
# the refit does, from the sdreport. bridge_se reads the log-scale standard deviation and returns
# it on the natural scale, so the doubled spawning biomass gives the doubled interval with it
p_ts_ref <- bridge_ts_figure(
  yrs = yrs,
  ssb = ssb_ref,
  rec = rec_ref,
  admb_ssb = s3$SSB,
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_ts_refit.png"),
  p_ts_ref,
  width = 15,
  height = 10,
  dpi = 170
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_sel_comparison.png"),
...
growth_df <- dplyr::bind_rows(lapply(c(1977, 2000, 2005, 2010, 2016, 2023), function(y) {
  ...
  dplyr::bind_rows(
    data.frame(
      Age = ages,
      value = r$mean_LAA_spawn[1,1,iy,1,,1],
      Gear = paste("Year", y),
      type = "SPoRC"
    ),
    data.frame(
      Age = ages,
      value = as.numeric(gs[gs$Yr == y, as.character(ages)]),
      Gear = paste("Year", y),
      type = label
    )
  )
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_growth_comparison.png"),
...
tv_df <- dplyr::bind_rows(
  data.frame(
    Year = yrs,
    value = r$growth_pars_y[1,1,,1,1],
    Par = "Length at age 1.5 (cm)",
    type = "SPoRC"
  ),
  data.frame(
    Year = yrs,
    value = gy$L1[match(yrs, gy$Yr)],
    Par = "Length at age 1.5 (cm)",
    type = label
  ),
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_growth_tv.png"),
  p_tv,
  width = 12,
  height = 9,
  dpi = 170
...
comp_rows <- function(y, fleet, gear) {
  ...
  dplyr::bind_rows(
    ...
    data.frame(
      Age = dat_lens,
      value = ld$Exp[ld$Yr == y & ld$Fleet == fleet],
      Gear = gear,
      type = label
    )
  )
...
ggplot2::ggsave(
  here("vignettes", "figures", "ae_ebs_pcod_comp_comparison.png"),
