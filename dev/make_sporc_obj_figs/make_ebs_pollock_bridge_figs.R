# Purpose: Bridge the 2024 EBS walleye pollock assessment to SPoRC and render its figures
...
input_list <- Setup_Mod_Rec(
  ...
  # SR_ref_yr points spawning biomass per recruit at terminal year biologicals, matching the
  # assessment. built from the first year instead, Bzero is 6021.33 rather than 6109.5576
  rec_model = "ricker_rec",
  rec_lag = 1,
  SR_ref_yr = n_yrs,
  # do_rec_bias_ramp = 0 sets the ramp to 1 throughout, centering the penalty on -sigmaR^2/2.
  # an NA bias_year leaves the ramp at zero and the bias correction absent altogether
  do_rec_bias_ramp = 0,
  sigmaR_switch = 1,
  sigmaR_spec = "fix",
  # three separate sigmas in pm.tpl, one per penalty: sigr fixed at 1 for the stock-recruit
  # residuals, weights 0.1 and 1 for the initial ages and recruitment level, as sigma = 1/sqrt(2w)
  ...
  # pm.tpl has a second recruitment statement alongside the stock-recruit residuals: a sum of
  # squares on all log recruitments about their own mean, weight 1, which is sigma = 1/sqrt(2)
  Use_rec_level_pen = 1,
...
input_list <- Setup_Mod_Biologicals(
  ...
  # pm.tpl weights the multinomial by the raw observed composition with MN_const inside the log only,
  # which is comp_const_obs = 0. addtocomp must stay positive, since at zero the offset gives 0*log(0)
  addtocomp = 1e-3,
  comp_const_obs = 0,
  addtosrvidx = 0.01,
  addtofishidx = 0
...
# the deviations have all of log F) plus Fdev_pen_center = "own_mean".
...
input_list <- Setup_Mod_Fishsel_and_Q(
  ...
  # nested [[fleet]][[block]] then the bin groups. ages 12-15 share one coefficient, pm.tpl's flat
  # tail, and the deviations share the grouping so none is estimated on a bin with no coefficient
...
input_list <- Setup_Mod_Srvsel_and_Q(
  ...
  # q is a multiplicative scalar, so it can be solved rather than estimated. "geo" is the lognormal
  # maximum likelihood value; "arith" is what pm.tpl applies to the trawl survey, a bridged convention
  srv_q_type = c("arith", "est", "est", "geo"),
  # a weight of zero makes the objective skip the process error likelihood for ln_srvsel_devs, which
  # pm.tpl shapes with explicit penalties instead. that is also why srvsel_pe_pars_spec is "fix"
...
# pm.tpl's selectivity penalties: a fishery dome over ages 6-12, curvature at the change years, and
# a random walk at sd 0.5 except two years at 1.9. surveys get a difference and a dome penalty
...
# Mapping --------------------------------------------------------------------
# setup already unmaps a parametric fleet's unused deviation slots. what is left is the data
# window and the deviation groupings, which are levels here rather than pm.tpl's increments
...
# the first year is redundant with the coefficients and stays at zero, and bins within a group move
# together. built explicitly because est_shared_b would share a group across years too
...
# pm.tpl applies sel_devs_fsh(ii) from yrs_ch_fsh(ii) to the next year, so the 59 changes land on
# 1966:2024. mapping them a year early drops the last increment from the walk
...
# pm.tpl predicts the vessel of opportunity index from sel_ats itself, so that fleet shares the
# acoustic deviations. leaving fleet 3 without any freezes its selectivity while fleet 2 moves
map_ats[1, , , 1, 3] <- map_ats[1, , , 1, 2]
mapping$ln_srvsel_devs <- factor(map_ats)
data$map_ln_srvsel_devs <- map_ats

# pm.tpl keeps acoustic age 1 log selectivity at zero rather than estimating it, so it is fixed here
# too. it has no base logistic for the trawl survey either: the deviations are the parameters
...
# exp(par + dev) rescaled by its own mean is invariant to shifting par and dev together, so the
# level has to be fixed or the likelihood is flat along it. this is what pm.tpl's avgsel penalty does
...
# pm.tpl penalizes the fishery selectivity increments with weight 1, sigma = 1/sqrt(2) on the walk,
# and the trawl age 1 series' first differences with weight 8
...
# recruitment deviations are stock-recruit residuals here, and each year's residual depends on the
...
# the two estimated catchabilities come from the ratio of the pollock predicted index to SPoRC's,
# exact because q enters multiplicatively. fleets 1 and 4 solve theirs analytically
r <- obj$report(free)
...
ggplot2::ggsave(
  here("vignettes", "figures", "f_ebs_pol_ts_comparison.png"),
  bridge_ts_figure(
                  yrs = yrs,
                  ssb = ssb,
                  rec = rec,
                  admb_ssb = dat$admb$SSB,
                  admb_rec = dat$admb$Rec,
...
ggplot2::ggsave(
  here("vignettes", "figures", "f_ebs_pol_fishsel_comparison.png"),
  p_sel,
  width = 12,
  height = 7,
  dpi = 150
