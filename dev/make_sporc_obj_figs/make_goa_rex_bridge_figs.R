# Purpose: Bridge the 2025 GOA rex sole assessment (Model 25.1) to SPoRC and render its figures
# Creator: Matthew LH. Cheng
# Date Created: 8/22/26
#
# the specification is written out here rather than sourced; the check at the foot compares it
# against tests/testthat/helper-bridge_goa_rex.R so the two cannot fall out of step
...
# recruitment: age 0 at the start of the year, apportioned across the two areas by an estimated
# logit. ramp breakpoints are in deviation-index space, so the two before the model start are negative
...
# Males have none, since spawning biomass is female only.
...
# biology: natural mortality fixed, growth estimated per area and sex with weight at age derived
# from the key, the assessment's ageing error, and its composition constant on both sides
...
# the assessment solves fishing mortality from the catch under its hybrid method, so here it is a
# free parameter per year with no penalty, kept to the catch by a tight lognormal error
...
# fishery: no index, joint-sex marginal ages and lengths, no conditional age-at-length. its key and
# weight at age are read at mid season, where the assessment reads the catch's
...
# surveys: a biomass index in each area, joint-sex lengths, and conditional age-at-length split by
# sex. the assessment keeps the surveys' marginal ages as ghost fleets, so they go in but are not fit
...
# selectivity is the age-based double normal everywhere, time invariant, with the male curve an
# offset from the female. the assessment leaves the ascending limb unanchored, which raw has
...
# catchability: the Western-Central survey estimates its own under the assessment's normal prior on
# the log scale, and the Eastern survey mirrors it, which the seeding expresses through the map
q_prior <- data.frame(
  region = 1,
  fleet = 1,
  block = 1,
  mu = exp(dat$q$prior_mean),
  sd = dat$q$prior_sd
)
...
# deviations run through 2022; the two later years have none and are mapped off.
...
# the early deviations set the start year's ages 1-17: the deviation for year styr - a lands on age
# a, less its bias correction. older ages have none, and the deviation is shared across areas and sexes
...
# fishing mortality as a fixed mean plus deviations in the area with the fishery. the other area's
# mean sits at a rate that removes nothing and its deviations stay mapped off
...
# selectivity: female parameters into the first sex's slots, male offsets into the second's. the
# assessment offsets only the peak, the two widths and the last bin, and the map levels the rest
...
# catchability: the assessment mirrors the Eastern survey's q on the Western-Central survey's, which
# the map expresses by giving the two cells one level so they hold one parameter
...
# Stage 1 comparison. The assessment's report has six significant digits, so
...
# standard errors on the natural scale. log quantities are reported in (population, area, year)
# order with a leading dim of one, so entries lay out area fastest then year
...
p_ts <- bridge_ts_figure(
  yrs = yrs,
  ssb = ssb_tot,
  rec = rec_tot,
  admb_ssb = ss3_ssb_tot,
  admb_rec = ss3_rec_tot,
  label = label,
  ssb_se = ssb_se_tot,
  rec_se = rec_se_tot
)
ggplot2::ggsave(
  here("vignettes", "figures", "ad_goa_rex_ts_comparison.png"),
  p_ts,
  width = 17,
  height = 9,
  dpi = 200
...
for(a in 1:n_reg) {
  ...
  pd_rows <- c(pd_rows, list(
    data.frame(
      Year = yrs,
      value = 100 * (ssb_area[a, ] - dat$ss3$SSB[, a]) / dat$ss3$SSB[, a],
      Par = "Spawning Biomass",
      Area = area_names[a]
    ),
    data.frame(
      Year = yrs,
      value = 100 * (rec_area[a, ] - dat$ss3$Rec[, a]) / dat$ss3$Rec[, a],
      Par = "Recruitment",
      Area = area_names[a]
    )
  ))
...
ggplot2::ggsave(
  here("vignettes", "figures", "ad_goa_rex_ts_comparison_area.png"),
...
ggplot2::ggsave(
  here("vignettes", "figures", "ad_goa_rex_sel_comparison.png"),
  bridge_sel_figure(sel_df, nrow = 3),
  width = 12,
  height = 12,
  dpi = 170
)
...
ggplot2::ggsave(
  here("vignettes", "figures", "ad_goa_rex_growth_comparison.png"),
...
# the regression test builds this same model from the test helper. the two specifications are
# written out separately, so compare them rather than trusting that they still agree
