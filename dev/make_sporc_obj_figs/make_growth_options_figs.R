# Purpose: Render the figures for vignette("af_growth_options"), the guide to the Setup_Mod_Biologicals() growth switches
...
#   3. time variation, growth_tv_type curve against cohort, on the EBS Pacific cod model
#   4. a semi-parametric surface recovered from simulated data, against the parametric fit
#
# reference ages in figure 1 are chosen so the switch is visible: a species already at its
# asymptote by the second reference age would show nothing
...
## 1. The curve --------------------------------------------------------------
# one parameter set throughout, in the Schnute form the module reads: length at a young reference
# age, length at an old one, the rate joining them, and L0 anchoring the linear phase below A1
...
p_anat <- ggplot(anat, aes(Age, L)) +
  ...
  annotate(
    ...
    x = c(0, A1, A2) + 1.5,
    y = c(L0, L1, L2) - 5,
    label = c("L[0]", "L[1]~at~A[1]", "L[2]~at~A[2]"),
    parse = TRUE,
    size = 5
  ) +
...
# (c) growth_A2: L2 read either as the length at a reference age, from which the asymptote is
# solved, or as the asymptote itself. the reference age is young enough that the curve still climbs
A2_y <- 8
...
a2_df <- bind_rows(
  ...
  data.frame(
    Age = ages,
    L = get_laa_curve(ages, L0, L1, L2, K, CV1, CV2, A1, max(ages),
                                           L2_asymptote = 1)$L,
...
# (d) growth_plus_group: the accumulator age holds every older fish, so its mean size is a mixture
# rather than the curve's value. an accumulator well short of the asymptote separates the two
acc <- 8
...
p_pg <- ggplot(pg_df, aes(Age, L, color = Option, linetype = Option)) +
  ...
  annotate(
    "text",
    x = acc - 1.2,
    y = mean(c(crv_acc$L[acc + 1], pg[acc + 1])),
    label = sprintf("%.1f", pg[acc + 1] - crv_acc$L[acc + 1]),
    hjust = 1,
    size = 4.6
  ) +
  ggthemes::scale_color_colorblind() +
...
ggsave(
  file.path(fig_dir, "af_growth_curve.png"),
...
cv_df <- bind_rows(
  ...
  data.frame(
    Age = ages,
    cv = get_laa_curve(ages, L0, L1, L2, K, CV1, CV2, A1, A2, cv_type = 1)$cv,
...
sd_df <- bind_rows(
  ...
  data.frame(
    Age = ages,
    sd = get_laa_curve(ages, L0, L1, L2, K, CV1 = 3, CV2 = 5, A1, A2, sd_type = 1)$sd,
...
# (c) growth_dist: the distribution the key bins. the forms are close at a modest CV, so this panel
# is drawn at a wide one, where the lognormal's skew and floor at zero change the key
#
# get_alk() always gets the spread growth_sd_type produced, so only two pairings are dimensionally
# coherent: normal with "cv", lognormal with "sd". a log SD of 0.35 is nearly a CV of 0.35
cv_wide <- 0.35
...
p_dist <- ggplot(dist_df, aes(Length, p, color = Option, linetype = Option)) +
  ...
  labs(
    ...
    color = "Spread of 0.35",
    linetype = "Spread of 0.35"
  ) +
  thm + theme(legend.direction = "vertical")

# (d) the key itself, which the length compositions and conditional age-at-length rows are read
# through. on a square root scale, since the age zero column is nearly a spike
...
ggsave(
  file.path(fig_dir, "af_growth_spread.png"),
...
## 3. Time variation ----------------------------------------------------------
# the EBS Pacific cod model: independent annual deviations on length at age 1.5 and on K from
# 2000, with size at age advanced cohort by cohort
...
# (b) what curve and cohort do differently. the assessment's own deviations nearly coincide, so
# this panel exaggerates them into a sustained drop in K from 2000. not the assessment's estimate
step_inp <- p_inp
...
laa_df <- bind_rows(lapply(c(1999, 2003, 2012), function(y) {
  ...
  bind_rows(
    data.frame(
      Age = pages,
      L = step_cur$mean_LAA_spawn[1, 1, iy, 1, , 1],
      Option = 'growth_tv_type = "curve"'
    ),
    data.frame(
      Age = pages,
      L = step_coh$mean_LAA_spawn[1, 1, iy, 1, , 1],
      Option = 'growth_tv_type = "cohort"'
    )
...
ggsave(
  file.path(fig_dir, "af_growth_tv.png"),
...
# simulated data whose mean length at age moves by a known year-by-age surface. one model fits the
# parametric curve alone, the other adds a 2D AR(1) surface, both on the same comps and CAAL
...
ggsave(
  file.path(fig_dir, "af_growth_semipar.png"),
