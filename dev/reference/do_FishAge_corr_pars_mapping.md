...
Constructs factor maps for `FishAge_corr_pars` (region- and sex-specific
AR1 and sex correlation parameters) and `FishAge_corr_pars_agg`
...
``` r
do_FishAge_corr_pars_mapping(input_list)
```
...
The input `input_list` with elements `map\$FishAge_corr_pars` and
`map\$FishAge_corr_pars_agg` set to factor vectors.

## Details

Parameters are activated only when `FishAgeComps_LikeType` is in
