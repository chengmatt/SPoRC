...
Constructs factor maps for `ln_FishAge_theta` (fleet- region- and
sex-specific overdispersion) and `ln_FishAge_theta_agg` (aggregated
overdispersion) based on the composition type and likelihood specified
in `$data$FishAgeComps_Type` and `$data$FishAgeComps_LikeType`.
...
``` r
do_FishAge_theta_mapping(input_list)
```
...
- input_list:
  ...
  `FishAgeComps_Type`, `FishAgeComps_LikeType`, and `UseFishAgeComps` to
  be set by
  [`Setup_Mod_FishIdx_and_Comps`](https://chengmatt.github.io/SPoRC/dev/reference/Setup_Mod_FishIdx_and_Comps.md).

## Value

The input `input_list` with `$map$ln_FishAge_theta` and
`$map$ln_FishAge_theta_agg` set to factor vectors. Active parameters
