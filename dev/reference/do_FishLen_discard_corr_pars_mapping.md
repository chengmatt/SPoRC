...
Constructs factor maps for `FishLen_discard_corr_pars` (region- and
sex-specific AR1 and sex correlation parameters) and
`FishLen_discard_corr_pars_agg` (aggregated correlation parameters) for
1D and 2D logistic-normal len composition likelihoods.

## Usage

``` r
do_FishLen_discard_corr_pars_mapping(input_list)
```
...
The input `input_list` with elements `map\$FishLen_discard_corr_pars`
and `map\$FishLen_discard_corr_pars_agg` set to factor vectors.

## Details

Parameters are activated only when `FishLenComps_discard_LikeType` is in
