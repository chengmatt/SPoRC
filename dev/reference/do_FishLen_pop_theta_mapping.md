...
Constructs factor maps for `ln_FishLen_pop_theta` (fleet- region- and
sex-specific overdispersion) and `ln_FishLen_pop_theta_agg` (aggregated
overdispersion) based on the composition type and likelihood specified
in `$data$FishLenComps_pop_Type` and `$data$FishLenComps_pop_LikeType`.
...
``` r
do_FishLen_pop_theta_mapping(input_list)
```
...
- input_list:
  ...
  `FishLenComps_pop_Type`, `FishLenComps_pop_LikeType`, and
  `UseFishLenComps_pop` to be set by
  [`Setup_Mod_FishIdx_and_Comps`](https://chengmatt.github.io/SPoRC/dev/reference/Setup_Mod_FishIdx_and_Comps.md).

## Value

The input `input_list` with `$map$ln_FishLen_pop_theta` and
`$map$ln_FishLen_pop_theta_agg` set to factor vectors. Active parameters
