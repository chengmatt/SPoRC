...
Constructs factor maps for `SrvAge_pop_corr_pars` (region- and
sex-specific AR1 and sex correlation parameters) and
`SrvAge_pop_corr_pars_agg` (aggregated correlation parameters) for 1D
and 2D logistic-normal age composition likelihoods.

## Usage

``` r
do_SrvAge_pop_corr_pars_mapping(input_list)
```
...
The input `input_list` with elements `map\$SrvAge_pop_corr_pars` and
`map\$SrvAge_pop_corr_pars_agg` set to factor vectors.

## Details

Parameters are activated only when `SrvAgeComps_pop_LikeType` is in
