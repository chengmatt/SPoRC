...
Constructs factor maps for `ln_SrvAge_pop_theta` (fleet- region- and
sex-specific overdispersion) and `ln_SrvAge_pop_theta_agg` (aggregated
overdispersion) based on the composition type and likelihood specified
in `$data$SrvAgeComps_pop_Type` and `$data$SrvAgeComps_pop_LikeType`.
...
``` r
do_SrvAge_pop_theta_mapping(input_list)
```
...
- input_list:
  ...
  `SrvAgeComps_pop_Type`, `SrvAgeComps_pop_LikeType`, and
  `UseSrvAgeComps_pop` to be set by
  [`Setup_Mod_SrvIdx_and_Comps`](https://chengmatt.github.io/SPoRC/dev/reference/Setup_Mod_SrvIdx_and_Comps.md).

## Value

The input `input_list` with `$map$ln_SrvAge_pop_theta` and
`$map$ln_SrvAge_pop_theta_agg` set to factor vectors. Active parameters
