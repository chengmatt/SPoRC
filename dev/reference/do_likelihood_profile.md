...
- Pooled data likelihoods:
  ...
  `CatchAA_nLL_df`, `DiscardAA_nLL_df` and `SrvIdxAA_nLL_df` with `_pop`
  variants, each summed over ages within a cell before its weight is
  applied `[Region × Year × Seas × Fleet]`, `FishAge_nLL_df`,
