...
Constructs the factor map for `ln_retsel_devs`, the annual deviations in
continuous time-varying retained fishery selectivity. For iid and
random-walk forms, the deviation dimension corresponds to selectivity
parameters (up to 6 for double-normal); for semi-parametric forms (3D
GMRF, 2D AR1), it corresponds to age or length bins. Cells with no
time-variation (`cont_tv_ret_sel == 0`) or no catch data are mapped to
`NA`.

## Usage

``` r
do_retsel_devs_mapping(
  input_list,
  ret_sel_devs_spec,
...
- ret_sel_devs_spec:
  ...
  :   Shared across bin groups defined by `retsel_devs_shared_bins`.
  ...
  semi-parametric time-varying forms (`cont_tv_ret_sel` is in
  `c(3,4,5)`).
...
- retsel_devs_shared_bins:
  ...
  `ret_sel_devs_spec` includes `"est_shared_b"`.
...
The input `input_list` with `$map$ln_retsel_devs` set to a factor vector
and `$data$map_ln_retsel_devs` set to the corresponding integer array
(for use in the RTMB template).

## Details

Bin-sharing (`"est_shared_b"` and related options) groups bins into
blocks defined by `retsel_devs_shared_bins`, reducing the number of
...
`$data$map_ln_retsel_devs` for use in the RTMB objective function.
