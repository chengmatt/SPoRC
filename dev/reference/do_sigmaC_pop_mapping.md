...
Constructs the `ln_sigmaC_pop` factor map used by the TMB/RTMB objective
...
``` r
do_sigmaC_pop_mapping(input_list, sigmaC_pop_spec)
```
...
- input_list:
  ...
  `$data$n_fish_fleets`, and `$par$ln_sigmaC_pop` to be populated before
  calling.

- sigmaC_pop_spec:

  Character string controlling the sharing and estimation structure for
  `ln_sigmaC_pop`. One of:
