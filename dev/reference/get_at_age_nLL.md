...
retained catch at age, discard catch at age, and the survey index at
age, each in an aggregated and a population-specific form. They differ
only in which array supplies the prediction and which parameter supplies
the standard deviation, so the likelihood itself is written once.
