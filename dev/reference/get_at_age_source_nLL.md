...
- obs_t:
  ...
  Registered observations for this data source, one element per cell
  flagged in `use`, in [`which()`](https://rdrr.io/r/base/which.html)
...
- pop:
  ...
  have a leading population dimension and whose observations are never
...
- const:
  ...
  aggregated data source's convention.
...
- trans_rho:
  ...
  with a leading population dim when `pop` is `TRUE`.
...
- us_pars:
  ...
  Unconstrained correlation parameters, over pair by the dims of
...
Everything that can differ between fleets does: the dims summed over,
