...
value, so a deviation fixed at a value is a year the walk passes through
rather than a gap in the series. A year whose own deviation is fixed
contributes no density.

## Usage

``` r
get_dev_pe_nLL(devs, is_est, sigma, dev_mu, PE_model, rho = 0, init_sd = 5)
```
...
- devs:
  ...
  Numeric vector of deviations for one series, by year.
