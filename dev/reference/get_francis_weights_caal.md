...
pooled over every row a fleet has, which gives one weight per fleet (per
region and sex where the composition type splits them). A weight per
length bin would rest on far fewer samples and would not be stable.
