...
its maximum likelihood estimates, and its derived quantities and
...
share one), `mle` (the assessment's maximum likelihood estimates, read
from its parameter file at twelve significant digits), and `ss3` (its
derived quantities and likelihood components for bridge comparison)
