...
given. A value of the wrong shape is not rejected by that: it is passed
...
``` r
use_starting_value(default, starting_values, par_name)
```
...
`starting_values[[par_name]]` when it is supplied and correctly shaped,
otherwise `default`.
