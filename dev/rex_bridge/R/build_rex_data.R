# Helper functions to build SS3 data into SPoRC
library(r4ss)
...
build_rex_data <- function(run_dir) {
  ...
  for(i in seq_len(nrow(d$catch))) {
    ci <- d$catch[i, ]
    if(ci$year < d$styr) next # skip the -999 equilibrium catch row
  ...
  # (sex-specific rows, one length bin each, counts of otoliths). Ghost fleets are stored but never used.
  ac <- d$agecomp
  ...
  # Ageing error from SS3 gives mean (-1 = unbiased, read as true age + 0.5) and SD of the
...
add_rex_ss3_report <- function(dat, report) {
  ...
  growth_mle <- array(NA, dim = c(1, n_regions, n_sexes, 5), dimnames = list(NULL, NULL, c("Fem", "Mal")[1:n_sexes], c("L1", "L2", "K", "CV1", "CV2")))
  for(s in 1:n_sexes) for(gp in 1:n_regions) {
    sx <- c("Fem", "Mal")[s]
