# Purpose: certified PML sandeel model, expect jnLL 1842.267 with pdHess TRUE and no NaN SEs
# Date Created: 9/3/26
...
# setup movement
# unstructured proportions with a dirichlet penalty, every cell estimated
input_list <- Setup_Mod_Movement(
  ...
  # one row per origin on a moving age, age index 1 is held at identity
  Movement_prior = expand.grid(pop = 1,
                               region_from = 1:n_regions,
                               age = 2,
                               year = 1,
                               seas = 1,
                               sex = 1,
                               alpha = I(list(c(1.5, 1.5, 1.5))))
)

# start with little movement, the uniform default scrambles the population
input_list$par$move_pars[] <- -3
...
# save stuff
saveRDS(input_list, here("dev", "spatial_sandeel", "inputs", "spatial_input_list_pml.rds"))



# fit ---------------------------------------------------------------------

# penalized likelihood, all fixed effects
mod_est <- fit_model(
  input_list$data,
  input_list$par,
  input_list$map,
...
# polish with restarts, keep the best point
best <- mod_est$optim
for(i in 1:3) {
  cand <- stats::nlminb(best$par, mod_est$fn, mod_est$gr,
                        control = list(iter.max = 1e5, eval.max = 1e5, rel.tol = 1e-15))
  if(cand$objective <= best$objective) best <- cand
} # end i loop
mod_est$optim <- best
mod_est$rep <- mod_est$report(best$par)

# tape hessian for the sdreport
sd_rep <- sdreport(mod_est, hessian.fixed = mod_est$he(best$par))

saveRDS(list(optim = best, sd_rep = sd_rep, rep = mod_est$rep),
        here("dev", "spatial_sandeel", "inputs", "pml_certified_fit.rds"))
