# Purpose: set up the 3 region North Sea sandeel model from the munged inputs
# Date Created: 9/3/26
...
input_list$par$ln_sigmaR[] <- log(1) # same sigR in all regions
...
input_list <- Setup_Mod_Movement(
  ...
  Fixed_Movement = NA,
  do_recruits_move = 0, # force recruits to stay put
  move_type = 1,
  move_timing = 2,
...
# save stuff
saveRDS(input_list, here("dev", "spatial_sandeel", "inputs", "spatial_input_list.rds"))
...
mod_est$gr(mod_est$env$last.par.best)

plot(mod_est$rep$SSB[1,1,], type = 'l', ylim = c(0, 1e6))
lines(mod_est$rep$SSB[1,2,], type = 'l')
lines(mod_est$rep$SSB[1,3,], type = 'l')

plot(mod_est$rep$Total_Biom[1,1,], type = 'l', ylim = c(0, 1e6))
lines(mod_est$rep$Total_Biom[1,2,], type = 'l')
lines(mod_est$rep$Total_Biom[1,3,], type = 'l')

get_catch_fits_plot(list(input_list$data), list(input_list$rep), 1)

plot(mod_est$rep$Rec[1,1,], type = 'l')
plot(mod_est$rep$Rec[1,2,], type = 'l')
plot(mod_est$rep$Rec[1,3,], type = 'l')

mod_est$rep$Movement[1,,,1,1,3,1]
mod_est$rep$Movement[1,,,1,2,3,1]

get_ts_plot(list(mod_est$rep), list(sd_rep), 1, do_ci = T)[[3]]
get_ts_plot(list(mod_est$rep), list(sd_rep), 1, do_ci = T)[[4]]

plot(apply(mod_est$rep$SSB, 3, sum))
mod_est$rep$SSB

plot(mod_est$rep$PredCatch[1,1,,1,1], type = 'l')
lines(mod_est$rep$PredCatch[1,2,,1,3], type = 'l')
lines(mod_est$rep$PredCatch[1,3,,1,5], type = 'l')

plot(mod_est$rep$PredCatch[1,1,,2,2], type = 'l')
lines(mod_est$rep$PredCatch[1,2,,2,4], type = 'l')
lines(mod_est$rep$PredCatch[1,3,,2,6], type = 'l')

plot(mod_est$rep$PredSrvIdx[1,1,,2,1], type = 'l')
lines(mod_est$rep$PredSrvIdx[1,2,,2,2], type = 'l')
lines(mod_est$rep$PredSrvIdx[1,3,,2,3], type = 'l')


# look at model fits
osa_caa <- get_osa(model = mod_est, data = input_list$data, index_source = "CatchAA")
osa_srv <- get_osa(model = mod_est, data = input_list$data, index_source = "SrvIdxAA")

p_caa <- plot_resids(osa_caa)
p_srv <- plot_resids(osa_srv)


plot(apply(mod_est$rep$SSB, 3, sum))
lines(apply(est$rep$SSB, 3, sum))
