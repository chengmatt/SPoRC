# A small single-region operating model with retention and discarding, to switch on the priors,
# penalties and selectivity forms no other test turns on.
#
# Discarding is needed because the discard mortality penalty only applies to cells with discard data,
# and retention because the retained-selectivity prior acts on ret_fixed_sel_pars.
#
...
objective_setup_sim <- local({
  ...
  function(n_lens = NULL) {
    ...
    size_age <- NULL
    if(!is.null(n_lens)) {
      one_year <- objective_setup_sizeage(n_lens, sim_list$n_ages)
...
#' @param n_lens Number of length bins. When given, the test setup has length
#'   compositions and fits them, which selectivity estimated over lengths requires.
#'
#' @keywords internal
objective_setup_input <- function(
  rec = list(),
  catch_f = list(),
  fishsel = list(),
  srvsel = list(),
  ...
  sim_obj <- objective_setup_sim(n_lens)
  ...
  input_list <- Setup_Mod_FishIdx_and_Comps(
    ...
    FishLenComps_LikeType = len_like,
  ...
  Setup_Mod_Weighting(
    ...
    Wt_Discard = 1,
    Wt_D = 1,
    Wt_FishAgeComps = fish_wt,
    Wt_FishLenComps = fish_wt,
    Wt_SrvAgeComps = srv_wt,
    Wt_SrvLenComps = srv_wt
  )
}
