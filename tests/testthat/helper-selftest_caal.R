# Shared routines for the conditional age-at-length self-tests. The operating model has length bins, a
# size-age transition built the way SS3 builds one, and a fixed number of otoliths per length bin.
#
# The estimation model fits marginal length compositions plus CAAL and no marginal age compositions, so
# every piece of age information reaches it through the CAAL likelihood.
...
caal_make_om <- function(
  ...
  sim_list <- Setup_Sim_Survey(
    sim_list = sim_list,
    srv_sel_input = replicate(n_sims, curve7(1, 3)),
    t_srv = array(0, dim = c(1, 1, 1)), # start of the season, where the test setup's key is read
...
#'
#' @param caal_like likelihood for the CAAL data sources.
#' @param caal_type composition type string for the CAAL data sources.
...
caal_build_input <- function(
  ...
  input_list <- Setup_Mod_FishIdx_and_Comps(
    ...
    FishLenComps_Type = paste0(ct, "_Year_1-terminal_Fleet_1"),
  ...
  input_list <- Setup_Mod_SrvIdx_and_Comps(
    ...
    SrvLenComps_Type = paste0(ct, "_Year_1-terminal_Fleet_1"),
