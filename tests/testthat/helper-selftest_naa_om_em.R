...
#' Operating model with a known numbers-at-age process error
#' @keywords internal
naaom_make_om <- function(
  NAA_re = "2dar1",
  sigmaNAA = 0.25,
  rho_age = 0.5,
  rho_year = 0.4,
...
#'
#' Strips the replicate dim the simulator has and peels to the terminal year.
#'
...
naaom_build_em <- function(sim_data, NAA_re = "none") {
  ...
  il <- Setup_Mod_Rec(
    ...
    do_rec_bias_ramp = 0,
    sigmaR_switch = 1,
    ln_sigmaR = array(log(naaom_cfg$sigmaR), c(2, 1, 1)),
    rec_model = "mean_rec",
    sigmaR_spec = "fix",
    init_age_strc = 1,
  ...
  il <- Setup_Mod_Biologicals(
    ...
    M_spec = "fix",
    Fixed_natmort = array(naaom_cfg$M, dim = c(1, 1, n_yrs, n_ages, 1)),
    NAA_re = NAA_re
  )
...
#' @keywords internal
naaom_fixed <- function(fit, quant_name = NULL) {
  p <- fit$env$last.par.best
  pf <- if(length(fit$env$random)) p[-fit$env$random] else p
  if(is.null(quant_name)) pf else unname(pf[names(pf) == quant_name])
}
