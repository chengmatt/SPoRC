# At-age data sources are stored over regions and sexes whatever a fleet reports. Each fleet names the
# dims it sums over, its density, where its observation error comes from and how its ages covary.
...
test_that("flagging a summed dim outside slot one is refused", {
  ...
  expect_error(build_at_age(
    n_regions = 2,
    ObsCatchAA = array(1, dim = dr),
    UseCatchAA = array(1, dim = dr),
    CatchAA_Type = "agg"
  ),
               "sums over regions")
  ds <- c(1, ny, 1, na, 2, 1)
  expect_error(build_at_age(
    n_sexes = 2,
    ObsCatchAA = array(1, dim = ds),
    UseCatchAA = array(1, dim = ds),
    CatchAA_Type = "spltRaggS"
  ),
               "sums over sexes")
  expect_error(build_at_age(
...
test_that("an array missing the sex dim is refused rather than reinterpreted", {
  ...
  expect_error(build_at_age(
    n_sexes = 2,
    ObsCatchAA = array(2e3, dim = flat),
    UseCatchAA = array(1, dim = flat)
  ),
               "not the correct dimension")
  # the standard errors are kept to the same shape as the observations
  expect_error(build_at_age(
    n_sexes = 2,
    ObsCatchAA = array(2e3, dim = full),
...
test_that("the observation error key and its starting values need the sex dim", {
  ...
  expect_error(build_at_age(
    ...
    UseCatchAA = use,
    CatchAA_Type = "spltRspltS",
    sigmaCAA_key = array(1:na, dim = c(na, 1))
  ),
  ...
  expect_error(build_at_age(
    ...
    UseCatchAA = use,
    CatchAA_Type = "spltRspltS",
    ln_sigmaCAA = array(log(0.3), dim = c(na, 1))
  ),
               "not the correct dimension")
})
...
test_that("an unstructured correlation matches its matrix and is guarded", {
  ...
  expect_error(build_at_age(
    n_yrs = 5,
    n_ages = na,
    ObsCatchAA = array(2e3, dim = few),
    UseCatchAA = array(1, dim = few),
    AgeObsCorr_catch = "us"
  ),
...
test_that("a dim a fleet never observes has no correlation", {
  ...
  # and the population-specific data source has a population dim of its own
  dp <- c(1, d)
  il2 <- build_at_age(
    n_regions = 2,
    n_sexes = 2,
    ObsCatchAA_pop = array(2e3, dim = dp),
    UseCatchAA_pop = array(1, dim = dp),
    CatchAA_pop_Type = "spltRspltS",
...
test_that("discards at age are the total discarded, in their own units", {
  ...
  il <- build_at_age(
    ObsCatchAA = array(2e3, dim = d),
    UseCatchAA = array(1, dim = d),
    ObsDiscardAA = array(50, dim = d),
    UseDiscardAA = array(1, dim = d),
    discard_units = "abd",
    dmr_spec = "fix"
  )
  r <- at_age_rep(il)
  ...
  # which is what the aggregated discard data source is also stated as
...
sim_at_age_om <- function(type, n_sexes = 2, n_sims = 40, seed = 42) {
  ...
  sl <- Setup_Sim_Rec(
    ...
    rinit_input = array(2, dim = c(sl$n_pop, sl$n_regions, sl$n_sims)),
    use_rinit = 1,
...
test_that("data sources with different dims, densities and correlations coexist", {
  ...
  # the catch data source reads each region and sex separately
...
test_that("the at-age fits plot holds the sex and the error the fit used", {
  ...
  il <- build_at_age(
    ...
    UseCatchAA = array(1, dim = d),
    CatchAA_Type = "spltRspltS",
    ObsCatchAA_SE = array(0.15, dim = d),
    CatchAA_sigma_form = "est_quadrature"
  )
  p <- get_at_age_fits_plot(list(il$data), list(at_age_rep(il)), "m1", data_source = "CatchAA")

  expect_s3_class(p, "ggplot")
  expect_equal(nrow(p$data), ny * na * 2)
  # the facet names the sex because this data source reports sexes separately. A
  # data source summing over them says so instead of naming a sex it is not about
...
test_that("an input list with an older shape is refused, not reinterpreted", {
  ...
  for(quant_name in c("ObsCatchAA", "UseCatchAA", "ObsCatchAA_SE")) {
    old <- il
    old$data[[quant_name]] <- array(1, dim = c(1, ny, 1, na, 1))
