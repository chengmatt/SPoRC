...
build_aa <- function(
  ...
  il <- Setup_Mod_SrvIdx_and_Comps(
    il,
    ObsSrvIdx = array(1e5, dim = c(1, n_yrs, 1, 1)),
...
test_that("an AR(1) across ages reduces to independence at rho zero", {
  ...
  # and a non-zero correlation changes the density
...
test_that("the across-age correlation is validated and set per data source", {
  expect_error(build_aa(AgeObsCorr_catch = "nope"), "Valid options: iid, 1dar1")

  # each data source is configured where its data are, so freeing one leaves the rest
  ...
  # one correlation per fleet, not one per data source
...
test_that("a model with no at-age data is unchanged by the routines", {
  ...
  # the aggregated data source holds the catch instead
  expect_true(sum(r$Catch_nLL) != 0)
})

test_that("an input list built before at-age observations existed still runs", {
  il <- build_aa(aa = FALSE)
  for(quant_name in c("ObsCatchAA", "UseCatchAA", "ObsDiscardAA", "UseDiscardAA",
...
test_that("the at-age and aggregated forms agree where the two say the same thing", {
  ...
  # data source fit, so both variants below are compared against the same dynamics
  ...
  aa <- build_aa(
    ObsCatchAA_in = ObsCatchAA,
    UseCatchAA_in = UseCatchAA,
