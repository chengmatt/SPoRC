...
test_that("bicubic fishery selectivity (Selex_Model == 8) wires correctly through Setup_Mod_Fishsel_and_Q and SPoRC_rtmb", {
  ...
  sim_list <- Setup_Sim_Dim(n_sims = 1, n_yrs = 12, n_regions = 1, n_ages = 8, n_lens = NULL,
  ...
  build_input_list <- function(fish_sel_model, fish_fixed_sel_pars_spec = "fix",
    ...
    input_list <- Setup_Mod_Rec(
      ...
      rec_model = "mean_rec", use_rinit = 1, sigmaR_spec = "fix",
      init_age_strc = 1, equil_init_age_strc = 2,
      ln_global_R0 = log(5), ln_rinit = log(2)
    )
    ...
    input_list <- do.call(Setup_Mod_Srvsel_and_Q, c(
      list(input_list = input_list, srv_sel_model = srv_sel_model,
          srv_fixed_sel_pars_spec = srv_fixed_sel_pars_spec, srv_q_spec = c("est_all")),
      srv_extra_args
    ))
  ...
  test_that("model builds, evaluates, and differentiates without error with a single bicubic block", {
    ...
    node_par <- matrix(c(-1.5, -0.2, 0.6, 1.2,
                         -0.8,  0.3, 0.9, 0.5,
                         -0.3,  0.1, 0.4, -0.2),
  ...
  test_that("survey bicubic model builds, evaluates, and differentiates without error with a single block", {
    n_bin_nodes <- 4; n_yr_nodes <- 3
    node_par <- matrix(c(-1.0, 0.3, 0.7, -0.2,
                         0.4, -0.5, 0.2, 0.9,
                         -0.6, 0.8, -0.1, 0.3),
                       nrow = n_yr_nodes, ncol = n_bin_nodes, byrow = TRUE)
    fixed_pars_init <- array(0, dim = c(1, n_bin_nodes * n_yr_nodes, 1, 1, 1))
  ...
  test_that("bicubic penalty terms are zero by default (no behavior change unless opted into)", {
    ...
    input_list_pen <- build_input_list(
      ...
      fish_fixed_sel_pars_spec = "est_all",
      fish_fixed_sel_pars = fixed_pars_init,
      fish_sel_pen_wts = list(smooth_bin_curve = 0, smooth_yr_diff = 0, smooth_yr_curve = 0, smooth_dome = 0, smooth_mean_center = 0)
    )
  ...
  test_that("setting bicubic penalty weights matches manual Get_Selex_Smoothness_Penalty computation", {
    ...
    expected_penalty <- Get_Selex_Smoothness_Penalty(sel_vals, wt_bin_curve = pen_wts$smooth_bin_curve, wt_yr_diff = pen_wts$smooth_yr_diff,
                                                     wt_yr_curve = pen_wts$smooth_yr_curve, wt_dome = pen_wts$smooth_dome,
  ...
  # Bridges an ADMB assessment's "selectivity held constant between change years" coefficient
  ...
  # terms used by the bicubic spline now generalized to apply here too (see SPoRC_rtmb.R's
  ...
  test_that("nonpar fishery selectivity with discrete blocks can opt into the modular smoothness penalty (pollock-style bridge)", {
    ...
    expected_penalty <- Get_Selex_Smoothness_Penalty(sel_vals, wt_bin_curve = pen_wts$smooth_bin_curve,
    ...
    # yr_diff is zero within each block (selectivity held constant) and only bites at the one
  ...
  test_that("a nonpar/blocked fleet with no fish_sel_pen_wts set is unaffected by the penalty-section generalization", {
    ...
    obj_default <- RTMB::MakeADFun(cmb(SPoRC_rtmb, input_list_default$data), parameters = input_list_default$par,
                                   map = input_list_default$map, silent = TRUE)

    input_list_explicit_zero <- build_nonpar(fish_sel_pen_wts = list(smooth_bin_curve = 0, smooth_bin_diff = 0,
                                                                     smooth_yr_diff = 0, smooth_yr_curve = 0,
                                                                     smooth_dome = 0, smooth_mean_center = 0))
    obj_explicit_zero <- RTMB::MakeADFun(cmb(SPoRC_rtmb, input_list_explicit_zero$data), parameters = input_list_explicit_zero$par,
