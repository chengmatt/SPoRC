...
#
# condition_closed_loop_simulations extends a fitted assessment forward and replicates it, and everything
# downstream inherits it. A silent error here gives advice computed from a stock that was not assessed.
#
# What can be checked without the right answer is that conditioning preserves what it was given: the
# historical period is the assessment, the projection extends it, and deterministic replicates agree.
...
conditioned_loop <- local({
  ...
  function() {
    ...
    cached <<- condition_closed_loop_simulations(
      closed_loop_yrs = closed_loop_cfg$closed_loop_yrs,
      n_sims = closed_loop_cfg$n_sims,
      m$data,
      m$parameters,
      m$mapping,
      sd_rep = m$sdrep,
...
test_that("conditioning extends the time series by the closed-loop years", {
  ...
  # every conditioned array holds the extended length on its year dim
  for(quant_name in c("WAA", "MatAA", "natmort", "R0", "h", "sexratio")) {
    expect_equal(dim(cl[[quant_name]])[3], want, label = sprintf("year extent of %s", quant_name))
  }
  # movement has a second region dim, so its year sits one place later
  expect_equal(dim(cl$Movement)[4], want)
  # and the simulation count on its last
  for(quant_name in c("WAA", "MatAA", "natmort")) {
    expect_equal(dim(cl[[quant_name]])[length(dim(cl[[quant_name]]))], closed_loop_cfg$n_sims,
                 label = sprintf("simulation extent of %s", quant_name))
  }
})
...
test_that("the historical period of the operating model is the assessment", {
  ...
  for(quant_name in c("WAA", "MatAA", "WAA_fish", "WAA_srv")) {
    expect_equal(as.vector(cl_slice(cl[[quant_name]], hist, 1)), as.vector(m$data[[quant_name]]),
                 tolerance = 1e-12, label = sprintf("%s over the historical period", quant_name))
  }
...
test_that("projected years hold the terminal year forward", {
  ...
  for(quant_name in c("WAA", "MatAA", "natmort", "WAA_fish")) {
    expect_equal(as.vector(cl_slice(cl[[quant_name]], (NH + 1):(NH + CL), 1)),
                 as.vector(cl_slice(cl[[quant_name]], rep(NH, CL), 1)), tolerance = 1e-12,
                 label = sprintf("%s in the projected years", quant_name))
  }
})
...
test_that("replicates share the conditioning they are not meant to differ in", {
  ...
  for(s in 2:closed_loop_cfg$n_sims) {
    for(quant_name in c("WAA", "MatAA", "natmort")) {
      expect_equal(as.vector(cl_slice(cl[[quant_name]], hist, s)),
                   as.vector(cl_slice(cl[[quant_name]], hist, 1)), tolerance = 1e-12,
                   label = sprintf("%s, simulation %d against simulation 1", quant_name, s))
    }
  }
})
...
test_that("the historical period survives conditioning at every dimension", {
  ...
  for(d in cl_dims) {
    ...
    for(quant_name in c("WAA", "MatAA")) {
      expect_equal(as.vector(cl_slice(out$cl[[quant_name]], hist, 1)),
                   as.vector(out$il$data[[quant_name]]), tolerance = 1e-12,
                   label = sprintf("%s over history, %s", quant_name, d$label))
    }
  }
})
...
test_that("the at-age data sources are extended over the closed-loop years", {
  ...
  for(quant_name in c("UseCatchAA", "UseDiscardAA", "UseSrvIdxAA",
              "ObsCatchAA_SE", "ObsDiscardAA_SE", "ObsSrvIdxAA_SE")) {
    arr <- out$cl[[quant_name]]
    expect_false(is.null(arr), label = sprintf("%s is present after conditioning", quant_name))
    if(is.null(arr)) next
    expect_equal(dim(arr)[2], want, label = sprintf("year extent of %s", quant_name))
  }
})
...
#' Condition and run a closed loop on a model with at-age catch
#'
#' @keywords internal
...
test_that("conditioning holds the fitted state across and extends it over the projection", {
  ...
  # gathers them, then kept at the terminal year over the projection
  expect_equal(dim(cl$sigmaNAA)[3], cl$n_yrs)
  fitted_sd <- exp(naaom_fixed(d$fit, "ln_sigmaNAA"))
  expect_equal(as.vector(cl$sigmaNAA[1, 1, n_hist, 1, 1, 1]), as.vector(fitted_sd), tolerance = 1e-8)
  expect_equal(cl$sigmaNAA[1, 1, cl$n_yrs, 1, 1, 1], cl$sigmaNAA[1, 1, n_hist, 1, 1, 1])
...
test_that("the conditioned operating model generates process error over the projection years", {
  ...
  eta <- sim_env$naa_eta_all[1, 1, , 1, , 1, 1]
  ages <- cl$naa_re_ages
