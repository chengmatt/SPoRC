...
# see it: the discard streams reconciled before their bins were parsed, and the
# survey streams reconciled after their use arrays had already been stored. Both
...
test_that("a restriction reaches the use flags through every setup path", {
  skip_if_not(exists("objective_fixture_sim"), "helper-objective_fixture.R not loaded")

  sim_obj <- objective_fixture_sim(NULL)
  sd <- simulation_data_to_SPoRC(sim_env = sim_obj, y = sim_obj$n_years, sim = 1)
  n_yrs <- sim_obj$n_years; n_ages <- sim_obj$n_ages
  keep <- 1:4
  bad_year <- 5

  # year 5 carries mass only outside the bins being fitted, on every stream
  ...
  input_list <- Setup_Mod_Dim(years = 1:n_yrs, ages = 1:n_ages, lens = sim_obj$n_lens,
                              n_regions = 1, n_sexes = 1, n_fish_fleets = 1, n_srv_fleets = 1,
                              n_pop = 1, natal_region = sim_obj$natal_region, verbose = FALSE)
  input_list <- Setup_Mod_Rec(input_list = input_list, do_rec_bias_ramp = 0, sigmaR_switch = 1,
                              ...
                              ln_global_R0 = log(5))
  input_list <- Setup_Mod_Biologicals(input_list = input_list, WAA = sd$WAA, MatAA = sd$MatAA,
                                      WAA_fish = sd$WAA_fish, WAA_srv = sd$WAA_srv, fit_lengths = 0,
                                      SizeAgeTrans = NA, AgeingError = sd$AgeingError, M_spec = "fix",
  ...
  suppressWarnings(input_list <- Setup_Mod_Catch_and_F(
    input_list = input_list, ObsCatch = sd$ObsCatch, UseCatch = sd$UseCatch, Use_F_pen = 1,
    sigmaC_spec = "fix", ln_sigmaC = sd$ln_sigmaC, ln_sigmaF = array(log(1), dim = c(1, 1, 1)),
    ObsDiscard = sd$ObsDiscard, UseDiscard = sd$UseDiscard,
    sigma_dmr_spec = "fix", dmr_mean_spec = "est_all", ln_sigmaD = sd$ln_sigmaD))

  suppressWarnings(input_list <- Setup_Mod_FishIdx_and_Comps(
    input_list = input_list,
    ObsFishIdx = sd$ObsFishIdx, ObsFishIdx_SE = sd$ObsFishIdx_SE, UseFishIdx = sd$UseFishIdx,
    ObsFishAgeComps = sd$ObsFishAgeComps, UseFishAgeComps = sd$UseFishAgeComps,
    ISS_FishAgeComps = sd$ISS_FishAgeComps,
    ObsFishLenComps = sd$ObsFishLenComps, UseFishLenComps = sd$UseFishLenComps,
    ISS_FishLenComps = sd$ISS_FishLenComps, fish_idx_type = "biom",
  ...
  suppressWarnings(input_list <- Setup_Mod_SrvIdx_and_Comps(
    input_list = input_list,
    ObsSrvIdx = sd$ObsSrvIdx, ObsSrvIdx_SE = sd$ObsSrvIdx_SE, UseSrvIdx = sd$UseSrvIdx,
    ObsSrvAgeComps = sd$ObsSrvAgeComps, UseSrvAgeComps = sd$UseSrvAgeComps,
    ISS_SrvAgeComps = sd$ISS_SrvAgeComps,
    ObsSrvLenComps = sd$ObsSrvLenComps, UseSrvLenComps = sd$UseSrvLenComps,
    ISS_SrvLenComps = sd$ISS_SrvLenComps, srv_idx_type = "biom",
  ...
  # no fewer. The planted year 5 is one of them; the fixture supplies others of
