...
test_that("nonparlog is non-parametric on the log scale and centered within each year", {
  ...
  sel <- function(y, tv = 2) Get_Selex(Selex_Model = 9, TimeVary_Model = tv, pars = pars,
                                       ln_seldevs = ln_seldevs, Region = 1, Year = y,
                                       Bin = 1:n_ages, Sex = 1)
  ...
  test_that("the level of the parameters is not identified, only their differences", {
    shifted <- Get_Selex(Selex_Model = 9, TimeVary_Model = 2, pars = pars + 3.7,
                         ln_seldevs = ln_seldevs, Region = 1, Year = 2, Bin = 1:n_ages, Sex = 1)
  ...
  test_that("it differs from the logit-scale non-parametric form", {
    logit_form <- Get_Selex(Selex_Model = 5, TimeVary_Model = 2, pars = pars,
                            ln_seldevs = ln_seldevs, Region = 1, Year = 2, Bin = 1:n_ages, Sex = 1)
  ...
  test_that("it is differentiable through an RTMB tape", {
    f <- function(p) {
      "c" <- RTMB::ADoverload("c")
      "[<-" <- RTMB::ADoverload("[<-")
      s <- Get_Selex(Selex_Model = 9, TimeVary_Model = 0, pars = p,
                     ln_seldevs = ln_seldevs, Region = 1, Year = 1, Bin = 1:3, Sex = 1)
      sum(s^2)
    }
...
test_that("bin overrides replace named bins with their own free deviations", {
  ...
  base <- function(bins = NULL) t(sapply(1:n_yrs, function(y)
    Get_Selex(Selex_Model = 0, TimeVary_Model = 0, pars = c(log(3), log(1.2)),
              ln_seldevs = ln_seldevs, Region = 1, Year = y, Bin = 1:n_ages, Sex = 1,
              bin_devs = bin_devs, bin_dev_bins = bins)))
  ...
  test_that("the override survives a form that standardizes internally", {
    with_ovr <- Get_Selex(Selex_Model = 9, TimeVary_Model = 0, pars = rep(0, n_ages),
                          ln_seldevs = ln_seldevs, Region = 1, Year = 1, Bin = 1:n_ages, Sex = 1,
                          bin_devs = bin_devs, bin_dev_bins = 1)
  ...
  test_that("it is differentiable through an RTMB tape", {
    f <- function(p) {
      ...
      s <- Get_Selex(Selex_Model = 0, TimeVary_Model = 0, pars = c(log(2), log(1)),
                     ln_seldevs = array(0, dim = c(1, 1, 3, 1, 1)),
                     Region = 1, Year = 1, Bin = 1:3, Sex = 1,
                     bin_devs = bd, bin_dev_bins = c(1, 2))
      sum(s^2)
    }
...
test_that("setup_sel_bin_devs builds the parameter, map and process error arrays", {
  ...
  input_list <- Setup_Mod_Dim(years = 1:12, ages = 1:7, lens = NA, n_regions = 1, n_sexes = 1,
                              n_fish_fleets = 1, n_srv_fleets = 2, n_seas = 1, n_pop = 1,
                              natal_region = 1, verbose = FALSE)
  input_list$data$n_proj_yrs_devs <- 0

  out <- SPoRC:::setup_sel_bin_devs(input_list, list(1, c(1, 2)), c("none", "rw"),
                                    prefix = "srv", n_fleets = 2, bins = 7)
