...
#
# Every packaged dataset is single-population, the tests that build n_pop > 1 only exercise truncate_yr, and
# the one worked example (r_natal-homing-pop-lrgr-rg.Rmd) has eval = FALSE on the chunk that fits.
#
# These are the first checks that evaluate and differentiate the natal-homing routines.
...
multi_pop_input <- function(n_pop, n_regions, natal_region, n_yrs = 10, n_ages = 6) {
  ...
  sweep_input(
    ...
    # the data sources are off: what is under test is the process model
