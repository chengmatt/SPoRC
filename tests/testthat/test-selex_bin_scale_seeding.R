# Starting values that are stated on a bin scale have to be read on the bin scale
# the model is actually using.
#
# A double normal carries its peak as a position among bins, so the setup seeds it
# at the middle of the bin vector rather than leaving it at zero, which would sit
# below the ascending limb. Which bin vector that is depends on whether
# selectivity is over ages or over lengths.
#
# These tests are written against the length scale because that is where the two
# vectors differ: on a model with ages 1-6 and lengths 20-100, a peak read off the
# wrong vector lands at 3.5, which is below every length bin the model has.
...
selex_seed_input <- function(selex_type = "length", n_lens = 9) {
  ...
  il <- Setup_Mod_Dim(n_pop = 1, years = seq_len(NY), ages = seq_len(NAG), lens = lens,
                      n_regions = 1, n_sexes = 1, n_seas = 1, n_fish_fleets = 1,
                      n_srv_fleets = 1, verbose = FALSE)
  il <- Setup_Mod_Rec(il, rec_model = "mean_rec", sigmaR_spec = "fix", do_rec_bias_ramp = 0,
                      init_age_strc = 1, ln_global_R0 = log(1e6))
  ...
  il <- suppressWarnings(Setup_Mod_Biologicals(
    il, WAA = array(1, dim = bd), WAA_fish = array(1, dim = c(bd, 1)),
    WAA_srv = array(1, dim = c(bd, 1)), MatAA = array(1, dim = bd),
    fit_lengths = 1, SizeAgeTrans = sa, M_spec = "fix",
  ...
  il <- suppressWarnings(Setup_Mod_Catch_and_F(
    il, ObsCatch = array(1e4, dim = c(1, NY, 1, 1)), UseCatch = array(1, dim = c(1, NY, 1, 1)),
    sigmaC_spec = "fix", sigmaF_spec = "fix"))

  off <- array(0, dim = c(1, NY, 1, 1))
  il <- do.call(Setup_Mod_FishIdx_and_Comps, list(
    input_list = il, ObsFishIdx = array(1e5, dim = c(1, NY, 1, 1)),
    ObsFishIdx_SE = array(0.2, dim = c(1, NY, 1, 1)), UseFishIdx = off, fish_idx_type = "none",
    ObsFishAgeComps = array(1 / NAG, dim = c(1, NY, 1, NAG, 1, 1)), UseFishAgeComps = off,
    ISS_FishAgeComps = array(100, dim = c(1, NY, 1, 1, 1)), FishAgeComps_LikeType = "none",
    ObsFishLenComps = array(1 / n_lens, dim = c(1, NY, 1, n_lens, 1, 1)),
    UseFishLenComps = array(1, dim = c(1, NY, 1, 1)),
    ISS_FishLenComps = array(100, dim = c(1, NY, 1, 1, 1)), FishLenComps_LikeType = "Multinomial",
  ...
  il <- suppressWarnings(Setup_Mod_SrvIdx_and_Comps(
    il, ObsSrvIdx = array(1e5, dim = c(1, NY, 1, 1)), ObsSrvIdx_SE = array(0.2, dim = c(1, NY, 1, 1)),
    UseSrvIdx = off, srv_idx_type = "none",
    ObsSrvAgeComps = array(1 / NAG, dim = c(1, NY, 1, NAG, 1, 1)), UseSrvAgeComps = off,
    ISS_SrvAgeComps = array(100, dim = c(1, NY, 1, 1, 1)), SrvAgeComps_LikeType = "none",
    ObsSrvLenComps = array(0, dim = c(1, NY, 1, n_lens, 1, 1)), UseSrvLenComps = off,
    ISS_SrvLenComps = array(0, dim = c(1, NY, 1, 1, 1)), SrvLenComps_LikeType = "none",
...
test_that("a length-based double normal seeds its peak among the length bins", {
  ...
  z <- Setup_Mod_Fishsel_and_Q(fx$il, fish_sel_model = "dbnrml_Fleet_1",
...
test_that("a length-based survey double normal seeds its peak among the length bins", {
  ...
  fx <- selex_seed_input()
  il <- Setup_Mod_Fishsel_and_Q(fx$il, fish_sel_model = "logist1_Fleet_1",
                                fish_fixed_sel_pars_spec = "est_all", fish_q_spec = "fix")

  z <- Setup_Mod_Srvsel_and_Q(il, srv_sel_model = "dbnrml_Fleet_1",
...
test_that("an unrecognized selectivity domain is rejected rather than left undefined", {
  ...
  expect_error(
    Setup_Mod_Fishsel_and_Q(fx$il, fish_sel_model = "logist1_Fleet_1",
