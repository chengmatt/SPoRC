...
make_selex_args <- function(n_ages = 6, n_sexes = 2, n_fleets = 1) {
  ...
  list(
    ...
    n_seas = 1, n_ages = n_ages, n_lens = 1, n_sexes = n_sexes, n_fleets = n_fleets
  )
}
...
test_that("a par sex offset evaluates the second sex at the first sex's parameters plus its stored offsets", {
  ...
  # offsets held at zero reproduce sex-shared parameters
...
test_that("setup_sel_sex_offset validates its options and maps scale parameters for the second sex only", {
  ...
  out <- setup_sel_sex_offset(base_input, c("par", "scale"), prefix = "fish", n_fleets = 2,
                              fleet_label = "fishery fleet", sel_model_arr = sel_model_arr,
                              cont_tv_mat = cont_tv_mat, max_blks = 1)
  ...
  expect_error(setup_sel_sex_offset(base_input, c("par", "none"), prefix = "fish", n_fleets = 2,
                                    fleet_label = "fishery fleet", sel_model_arr = sel_model_arr,
                                    cont_tv_mat = cont_tv_mat, max_blks = 1), "n_sexes > 1")
  ...
  expect_error(setup_sel_sex_offset(base_input, c("none", "scale"), prefix = "fish", n_fleets = 2,
                                    fleet_label = "fishery fleet", sel_model_arr = sel_model_nonpar,
                                    cont_tv_mat = cont_tv_mat, max_blks = 1), "mean-standardized")
})
...
test_that("an NSelBins plateau holds bins beyond it at the last computed value for parametric forms", {
  ...
  outs <- do.call(Get_Selex_Array, c(args, list(nselbins = nselbins, sex_par_offset = 0,
                                                sex_scale_offset = 1, sex_scale = sex_scale)))
...
test_that("Get_Init_NAA applies sex-specific initial deviations and broadcasts a 3-D array", {
  ...
  n_ages <- 4; n_sexes <- 2
  d <- list(
    n_regions = 1, n_pop = 1, n_sexes = n_sexes, n_ages = n_ages, n_seas = 1, n_fish_fleets = 1,
    seasdur = 1, rec_seas_prop = matrix(1, 1, 1),
    natmort = array(0.2, dim = c(1, 1, n_ages, n_sexes)),
    init_F = array(0, dim = c(1, 1, 1)), dmr = array(0, dim = c(1, 1, 1)),
    fish_sel = array(0.5, dim = c(1, 1, 1, n_ages, n_sexes, 1)),
    ret_sel = array(1, dim = c(1, 1, 1, n_ages, n_sexes, 1)),
    R0_r = matrix(1000, 1, 1), sexratio = array(0.5, dim = c(1, 1, n_sexes)),
    Movement = array(1, dim = c(1, 1, 1, 1, n_ages, n_sexes)), do_recruits_move = 0
  )
...
test_that("the initial age penalty scores one copy per parameter and pools its own-mean center across sexes", {
  ...
  n_ages <- 5
  base_args <- list(
    n_pop = 1, n_regions = 1, n_ages = n_ages, n_est_rec_devs = 3, rec_dd = 0,
    natal_region = 1, rec_region_prop_spec = 0, rec_region_prop = matrix(1, 1, 1),
    equil_init_age_strc = 2, init_age_devs_shared = NULL,
    ln_sigmaR = array(log(0.6), dim = c(2, 1, 1)), bias_ramp = rep(0, 3), sigmaR_switch = 1,
    ln_RecDevs = array(0, dim = c(1, 1, 3)), sigmaR2_early = matrix(0.36, 1, 1),
    sigmaR2_late = matrix(0.36, 1, 1), do_rec_bias_ramp = 0
  )
  ...
  expect_equal(as.numeric(shared$Init_Rec_nLL[1,1,,2]), rep(0, n_ages - 1)) # the second copy is not scored
...
test_that("do_InitDevs_mapping expands its single-sex logic across sexes per InitDevs_sex_spec", {
  ...
  make_input <- function() {
    n_ages <- 5
    list(
      data = list(n_pop = 1, n_regions = 1, n_sexes = 2, ages = 1:n_ages,
                  equil_init_age_strc = 2, rec_region_prop_spec = 0, natal_region = 1, rec_dd = 0),
  ...
  shared <- do_InitDevs_mapping(make_input(), InitDevs_spec = NULL, rec_dd = NULL,
                                init_age_devs_shared = NULL, InitDevs_sex_spec = "est_shared_s")
  ...
  distinct <- do_InitDevs_mapping(make_input(), InitDevs_spec = NULL, rec_dd = NULL,
                                  init_age_devs_shared = NULL, InitDevs_sex_spec = "est_all")
  ...
  fp <- do_InitDevs_mapping(fixed_plus, InitDevs_spec = NULL, rec_dd = NULL,
                            init_age_devs_shared = NULL, InitDevs_sex_spec = "est_all")
  ...
  expect_error(do_InitDevs_mapping(single, InitDevs_spec = NULL, rec_dd = NULL,
                                   init_age_devs_shared = NULL, InitDevs_sex_spec = "est_all"), "n_sexes > 1")
})

test_that("retention selectivity carries the sex offsets and the plateau through setup and the objective", {

  messages_list <<- character(0)
  # the helper builds retention's own flags and scale parameters
  base_input <- list(data = list(n_sexes = 2, n_regions = 1), par = list(), map = list())
  out <- setup_sel_sex_offset(base_input, "par_scale", prefix = "ret", n_fleets = 1,
                              fleet_label = "fishery fleet (retention)", sel_model_arr = array(0, dim = c(1, 3, 1)),
                              cont_tv_mat = array(0, dim = c(1, 1)), max_blks = 1)
  ...
  input_list <- Setup_Mod_Dim(years = 1:n_yrs, ages = 1:n_ages, lens = NULL, n_regions = 1, n_sexes = n_sexes,
                              n_fish_fleets = 1, n_srv_fleets = 1, n_seas = 1, n_pop = 1, natal_region = 1, verbose = FALSE)
  input_list <- Setup_Mod_Rec(input_list = input_list, rec_model = "mean_rec", sigmaR_spec = "fix",
  ...
  input_list <- Setup_Mod_Biologicals(input_list = input_list, WAA = biol(1:n_ages), MatAA = biol(rep(1, n_ages)),
                                      fit_lengths = 0, AgeingError = diag(n_ages), M_spec = "fix",
  ...
  suppressWarnings(input_list <- Setup_Mod_Catch_and_F(input_list = input_list, ObsCatch = array(1, dim = c(1, n_yrs, 1, 1)),
                                                       UseCatch = array(1, dim = c(1, n_yrs, 1, 1)), Use_F_pen = 0,
                                                       sigmaC_spec = "fix", ln_sigmaC = array(log(0.05), dim = c(1, n_yrs, 1, 1))))
  input_list <- Setup_Mod_FishIdx_and_Comps(
    input_list = input_list, ObsFishIdx = array(NA_real_, dim = c(1, n_yrs, 1, 1)), ObsFishIdx_SE = array(NA_real_, dim = c(1, n_yrs, 1, 1)),
    UseFishIdx = array(0, dim = c(1, n_yrs, 1, 1)), ObsFishAgeComps = array(NA_real_, dim = c(1, n_yrs, 1, n_ages, n_sexes, 1)),
    UseFishAgeComps = array(0, dim = c(1, n_yrs, 1, 1)), ISS_FishAgeComps = array(0, dim = c(1, n_yrs, 1, n_sexes, 1)),
    ObsFishLenComps = NULL, UseFishLenComps = array(0, dim = c(1, n_yrs, 1, 1)), ISS_FishLenComps = NULL,
    fish_idx_type = "none", FishIdx_LikeType = "lognormal", FishAgeComps_LikeType = "none", FishLenComps_LikeType = "none",
    FishAgeComps_Type = "none_Year_1-terminal_Fleet_1", FishLenComps_Type = "none_Year_1-terminal_Fleet_1")
  input_list <- Setup_Mod_SrvIdx_and_Comps(
    input_list = input_list, ObsSrvIdx = array(NA_real_, dim = c(1, n_yrs, 1, 1)), ObsSrvIdx_SE = array(NA_real_, dim = c(1, n_yrs, 1, 1)),
    UseSrvIdx = array(0, dim = c(1, n_yrs, 1, 1)), ObsSrvAgeComps = array(NA_real_, dim = c(1, n_yrs, 1, n_ages, n_sexes, 1)),
    UseSrvAgeComps = array(0, dim = c(1, n_yrs, 1, 1)), ISS_SrvAgeComps = array(0, dim = c(1, n_yrs, 1, n_sexes, 1)),
    ObsSrvLenComps = NULL, UseSrvLenComps = array(0, dim = c(1, n_yrs, 1, 1)), ISS_SrvLenComps = NULL,
    srv_idx_type = "none", SrvIdx_LikeType = "lognormal", SrvAgeComps_LikeType = "none", SrvLenComps_LikeType = "none",
    SrvAgeComps_Type = "none_Year_1-terminal_Fleet_1", SrvLenComps_Type = "none_Year_1-terminal_Fleet_1")
  input_list <- Setup_Mod_Fishsel_and_Q(input_list = input_list, fish_sel_model = "logist1_Fleet_1",
                                        ...
                                        use_fixed_ret_sel = 0, ret_sel_model = "logist1_Fleet_1_NSelBins_4",
                                        ret_fixed_sel_pars_spec = "est_all", ret_sel_sex_offset = "par_scale")
  input_list <- Setup_Mod_Srvsel_and_Q(input_list = input_list, srv_sel_model = "logist1_Fleet_1",
                                       srv_fixed_sel_pars_spec = "est_all", srv_q_spec = "fix")
  input_list <- Setup_Mod_Tagging(input_list = input_list, use_conv_fish_tagging = 0)
  input_list <- Setup_Mod_Weighting(input_list = input_list, Wt_Catch = 1, Wt_FishIdx = 0, Wt_SrvIdx = 0, Wt_Rec = 1, Wt_F = 1,
...
test_that("the between-sex tie on initial age deviations is a Gaussian on each later sex's difference from the first", {
  ...
  on <- do.call(get_recruitment_penalty, c(base_args, list(ln_InitDevs = dev4, init_devs_pen_use = pen_all,
                                                          Use_init_sex_pen = 1, ln_sigma_init_sex = log(1 / sqrt(2)))))
  ...
  on1 <- do.call(get_recruitment_penalty, c(args1, list(ln_InitDevs = dev4, init_devs_pen_use = pen_all,
                                                       Use_init_sex_pen = 1, ln_sigma_init_sex = 0)))
  expect_equal(as.numeric(on1$Init_Sex_nLL[1,1,n_ages - 1,2]), 0)

  # setup refuses the tie where it would be identically zero
  dim2 <- Setup_Mod_Dim(years = 1:5, ages = 1:4, lens = NULL, n_regions = 1, n_sexes = 2, n_fish_fleets = 1,
                        n_srv_fleets = 1, n_seas = 1, n_pop = 1, natal_region = 1, verbose = FALSE)
  expect_error(Setup_Mod_Rec(input_list = dim2, rec_model = "mean_rec", sigmaR_spec = "fix",
                             InitDevs_sex_spec = "est_shared_s", Use_init_sex_pen = 1), "est_all")
  ok <- Setup_Mod_Rec(input_list = dim2, rec_model = "mean_rec", sigmaR_spec = "fix",
                      InitDevs_sex_spec = "est_all", Use_init_sex_pen = 1, init_sex_pen_sigma = 1 / sqrt(2))
  expect_equal(ok$data$Use_init_sex_pen, 1)
  expect_equal(ok$data$ln_sigma_init_sex, log(1 / sqrt(2)))
  dim1 <- Setup_Mod_Dim(years = 1:5, ages = 1:4, lens = NULL, n_regions = 1, n_sexes = 1, n_fish_fleets = 1,
                        n_srv_fleets = 1, n_seas = 1, n_pop = 1, natal_region = 1, verbose = FALSE)
...
test_that("sex offsets refuse a sex-shared fixed specification under par and map scale offsets only for a fleet's own blocks", {
  ...
  expect_error(setup_sel_sex_offset(base_input, c("par", "none"), prefix = "fish", n_fleets = 2,
                                    fleet_label = "fishery fleet", sel_model_arr = sel_model_arr,
                                    cont_tv_mat = cont_tv_mat, max_blks = 2, sel_blocks = sel_blocks,
                                    fixed_spec = c("est_shared_s", "est_all")), "shares the sex slots")
  ok <- setup_sel_sex_offset(base_input, c("par", "none"), prefix = "fish", n_fleets = 2,
                             fleet_label = "fishery fleet", sel_model_arr = sel_model_arr,
                             cont_tv_mat = cont_tv_mat, max_blks = 2, sel_blocks = sel_blocks,
  ...
  out <- setup_sel_sex_offset(base_input, c("scale", "scale"), prefix = "fish", n_fleets = 2,
                              fleet_label = "fishery fleet", sel_model_arr = sel_model_arr,
                              cont_tv_mat = cont_tv_mat, max_blks = 2, sel_blocks = sel_blocks,
...
test_that("the simulator draws initial age deviations per sex only when asked, and the shared draw is unchanged", {
  ...
  make_sim <- function(spec, n_sexes = 2) {
    sim_list <- Setup_Sim_Dim(n_sims = 1, n_yrs = 8, n_regions = 1, n_ages = 5, n_lens = NULL,
                              n_sexes = n_sexes, n_fish_fleets = 1, n_srv_fleets = 1, n_pop = 1)
    sim_list <- Setup_Sim_Containers(sim_list)
    args <- list(sim_list = sim_list, R0_input = replicate(1, array(5, dim = c(1, 1, 8))),
                 ln_sigmaR = array(log(0.5), dim = c(2, 1, 1)), recruitment_opt = "mean_rec", init_age_strc = 1)
