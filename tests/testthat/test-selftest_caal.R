...
# its age information arrives through the CAAL likelihood. Machinery shared through
...
test_that("simulate_caal draws from P(age | length) and applies ageing error", {
  ...
  set.seed(3)
  out <- simulate_caal(r = 1, y = 1, f = 1, seas = 1, sim = 1, SizeAgeTrans = sat, AtAge = at_age,
                       ISS = iss, AgeingError = ae, comp_like = 0, ln_theta = array(0, c(1, 1, 1)),
                       ln_theta_agg = 0, comp_type = matrix(1, 1, 1), n_sexes = 1, n_regions = 1,
                       n_lens = n_lens, Obs = obs)
  ...
  out <- simulate_caal(r = 1, y = 1, f = 1, seas = 1, sim = 1, SizeAgeTrans = sat, AtAge = at_age,
                       ISS = iss, AgeingError = ae_shift, comp_like = 0, ln_theta = array(0, c(1, 1, 1)),
                       ln_theta_agg = 0, comp_type = matrix(1, 1, 1), n_sexes = 1, n_regions = 1,
                       n_lens = n_lens, Obs = obs)
  drawn <- out[1, 1, 1, , , 1, 1, 1]
...
test_that("Dirichlet-multinomial CAAL recovers the overdispersion it was generated under", {
  theta_true <- 0.5
  n_reps <- 20
  om <- caal_make_om(caal_like = "Dirichlet-Multinomial", ln_theta = log(theta_true), n_sims = n_reps, seed = 77)
...
test_that("two sexes drawn jointly within a length bin are recovered", {
  ...
  out <- caal_run(caal_build_input(sim_data, caal_type = "spltRjntS", n_sexes = 2),
                  what = c("SSB", "Rec"), n_sims = 30, seed = 303)
  expect_lt(abs(out$summ$SSB[["median_RE"]]), 0.03)
})
...
test_that("CAAL one step ahead residuals are standard normal under the generating model", {
  ...
  osa <- get_osa(model = fit, data = fit$data, comp_source = "Fish_caal",
                 bins = 1:caal_cfg$n_ages, bin_label = "Age")
  res <- osa$res
  expect_true("len" %in% names(res))
  expect_setequal(unique(res$len), caal_cfg$len_lower + 2.5)

  # The package's discrete residuals carry a small offset in their mean under a
  ...
  len_res <- get_osa(model = fit, data = fit$data, comp_source = "FishLen",
                     bins = 1:caal_cfg$n_lens, bin_label = "Len")$res$resid
