...
test_that("simulated innovations hold the correlation on the dim they name", {
...
test_that("a simulated region correlation appears across regions and leaves ages alone", {
  ...
  env <- naa_sim_env(
    NAA_re = 4,
    naa_rho = c(age = 0.7, year = 0.4, cohort = 0),
    NAA_re_region = 1,
    naa_region_corr = c(0.8, 0.2, 0.5)
  )
  D <- replicate(1500, SPoRC:::draw_naa_innovations(env), simplify = FALSE)
  rc <- function(i, j) mean(vapply(D, function(x) stats::cor(as.vector(x[1,i,,1,,1]), as.vector(x[1,j,,1,,1])), numeric(1)))
...
test_that("the penalty recovers the parameters the simulator drew from", {
  ...
  fit_reps <- function(code, sd_true, rho_true) {
    ...
    pred <- array(exp(5), dim = c(np, nr, ny, 1, na, ns))
