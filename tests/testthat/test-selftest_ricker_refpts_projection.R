...
# projection), so agreement pins both. Machinery shared through
...
test_that("a 500-year projection at the Ricker Fmsy equilibrates at Bmsy", {
  ...
  rp <- Get_Reference_Points(data = data, rep = rep, type = "single_region", what = "MSY",
                             calc_rec_st_yr = 1, rec_age = 1)
  ...
  natmortp <- array(0.3, dim = c(1, 1, n_proj_yrs, n_ages, 1))
  Movementp <- array(1, dim = c(1, 1, 1, n_proj_yrs, 1, n_ages, 1))

  srr_opt <- list(
    rec_dd = 1, rec_lag = 1, do_recruits_move = 0,
    R0 = rep$R0, h = array(rep$h_trans, dim = c(1, 1)),
    ...
    natmort = array(0.3, dim = c(1, 1, n_ages)),
  ...
  proj <- Do_Population_Projection(
    n_proj_yrs = n_proj_yrs, n_pop = 1, n_regions = 1, n_ages = n_ages, n_sexes = 1,
    sexratio = array(1, dim = c(1, 1, n_proj_yrs, 1)), n_fish_fleets = 1,
    do_recruits_move = 0, recruitment = rep$Rec, terminal_NAA = terminal_NAA,
    terminal_NAA0 = terminal_NAA0,
    terminal_F = array(rep$Fmort[1, n_yrs, 1, 1], dim = c(1, 1, 1)),
    natmort = natmortp, WAA = WAAp, WAA_fish = WAA_fishp, MatAA = MatAAp,
    fish_sel = fish_selp, ret_sel = array(1, dim = dim(fish_selp)),
    Movement = Movementp,
    f_ref_pt = array(rp$f_ref_pt, dim = c(1, n_proj_yrs)),
    b_ref_pt = array(rp$b_ref_pt, dim = c(1, 1, n_proj_yrs)),
    HCR_function = constant_F_HCR,
    recruitment_opt = "ricker_rec", fmort_opt = "HCR", t_spawn = 0,
    srr_opt = srr_opt
  )

  ssb_end <- proj$proj_SSB[1, 1, n_proj_yrs]
  expect_lt(abs((ssb_end - rp$b_ref_pt) / rp$b_ref_pt), 1e-6)
  # and it has actually equilibrated, rather than still drifting through Bmsy
