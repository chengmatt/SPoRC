# Self-test of the sex-linked selectivity options and sex-specific initial age
# deviations, end to end through the simulation module: a two-sex operating
# model whose truth IS the offset structure (a male fishery curve scaled by
# exp(-0.163), survey male parameters as log offsets on the female's, a
# selectivity plateau from bin 4, and per-sex initial age curves) generates
# joint-sex data, the estimation model carries the same structure through
# fish_sel_sex_offset/srv_sel_sex_offset/NSelBins/InitDevs_sex_spec, and
# simulation_self_test conditions the operating model on the fit, so the
# fitted per-sex surfaces and initial deviations travel back through
# Setup_Sim_Fishing/Setup_Sim_Survey/Setup_Sim_Rec. Follows the structure of
# helper-selftest_features.R, but self-contained because that helper's
# operating model is single-sex.
...
test_that("sex offsets, the selectivity plateau, and per-sex initial deviations recover through the simulation", {
  ...
  sim_list <- Setup_Sim_Dim(n_sims = 1, n_yrs = n_yrs, n_regions = 1, n_ages = n_ages,
                            n_lens = NULL, n_sexes = n_sexes, n_fish_fleets = 1, n_srv_fleets = 1, n_pop = 1)
  ...
  input_list <- Setup_Mod_Rec(input_list = input_list, do_rec_bias_ramp = 0, sigmaR_switch = 1,
                              ln_sigmaR = array(log(0.1), c(2, 1, 1)), rec_model = "mean_rec",
                              sigmaR_spec = "fix", init_age_strc = 1, equil_init_age_strc = 2,
                              InitDevs_sex_spec = "est_all", ln_global_R0 = log(5))
  ...
  input_list <- Setup_Mod_FishIdx_and_Comps(
    ...
    FishLenComps_LikeType = "none", FishAgeComps_Type = "spltRjntS_Year_1-terminal_Fleet_1",
  ...
  input_list <- Setup_Mod_SrvIdx_and_Comps(
    ...
    SrvLenComps_LikeType = "none", SrvAgeComps_Type = "spltRjntS_Year_1-terminal_Fleet_1",
  ...
  input_list <- Setup_Mod_Weighting(
    input_list = input_list, Wt_Catch = 1, Wt_FishIdx = 0, Wt_SrvIdx = 1,
    Wt_Rec = 1, Wt_Init_Rec = 1, Wt_F = 1,
  ...
  # both sexes' fitted curves carry the plateau (bin 5 rides bin 4)
  ...
  res <- simulation_self_test(
    ...
    n_sims = n_sims, what = c("SSB", "fish_sel", "srv_sel"), sim_recruitment = "input"
  )
  for(w in c("SSB", "fish_sel", "srv_sel")) { # res also carries an sd_rep slot, so iterate the names
    truth <- as.numeric(fit$rep[[w]])
    est_mat <- matrix(res[[w]], nrow = length(truth), ncol = n_sims)
    keep <- truth > 1e-8 # selectivity arrays carry structural zeros outside 1:n_yrs
