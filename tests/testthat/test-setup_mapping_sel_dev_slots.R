...
test_that("deviation slots beyond a fleet's selectivity form stay unmapped", {
  ...
  input_list <- Setup_Mod_Rec(
    ...
    do_rec_bias_ramp = 0,
    sigmaR_switch = 1,
    ln_sigmaR = array(log(0.4), c(2, 1, 1)),
    rec_model = "mean_rec",
    sigmaR_spec = "fix",
    init_age_strc = 1,
  ...
  suppressWarnings(input_list <- Setup_Mod_Biologicals(
    ...
    fit_lengths = 0,
    AgeingError = array(rep(diag(n_ages), each = n_yrs), dim = c(n_yrs, n_ages, n_ages)),
    M_spec = "fix",
  ...
  suppressWarnings(input_list <- Setup_Mod_Catch_and_F(
    input_list = input_list,
    ObsCatch = array(5, dim = c(1, n_yrs, 1, 1)),
  ...
  suppressWarnings(input_list <- Setup_Mod_SrvIdx_and_Comps(
    ...
    ObsSrvLenComps = NULL,
    UseSrvLenComps = array(0, dim = c(1, n_yrs, 1, 2)),
  ...
  test_that("a time-varying logistic fleet has exactly its two parameter slots", {
