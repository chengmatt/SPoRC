...
test_that("Setup_Mod_Rec encodes sr_R0_spec and maps ln_sr_R0 to match", {
  ...
  base <- Setup_Mod_Dim(years = 1:20, ages = 1:10, lens = NA, n_regions = 1, n_sexes = 1,
                        n_fish_fleets = 1, n_srv_fleets = 1, n_seas = 1, n_pop = 1,
                        natal_region = 1, verbose = FALSE)
  rec <- function(...) suppressWarnings(Setup_Mod_Rec(input_list = base, rec_model = "mean_rec",
                        rec_lag = 1, SR_ref_yr = 1, init_age_strc = 1, equil_init_age_strc = 2,
                        t_spawn = 0, ...))

  # ln_sr_R0 is only ever in the parameter vector when the curve owns a scale of
  # its own; under the other settings it is mapped off so it cannot drift.
...
test_that("Setup_Mod_Rec rejects a penalty that would be counted twice or has no scale", {
  ...
  # would score the same quantity twice.
  expect_error(
    suppressWarnings(Setup_Mod_Rec(base, rec_model = "bh_rec", rec_lag = 1, SR_ref_yr = 1,
      sr_penalty = "bh", sr_pen_sigma = 0.5, init_age_strc = 1, equil_init_age_strc = 2,
      t_spawn = 0, use_rinit = 0)),
    "only valid with rec_model")

  # "rinit" takes the curve's scale from a parameter that use_rinit = 0 leaves out.
  expect_error(
    suppressWarnings(Setup_Mod_Rec(base, rec_model = "mean_rec", rec_lag = 1, SR_ref_yr = 1,
      sr_penalty = "bh", sr_pen_sigma = 0.5, sr_R0_spec = "rinit", init_age_strc = 1,
      equil_init_age_strc = 2, t_spawn = 0, use_rinit = 0)),
    "needs use_rinit = 1")

  expect_error(
    suppressWarnings(Setup_Mod_Rec(base, rec_model = "mean_rec", rec_lag = 1, SR_ref_yr = 1,
      sr_penalty = "bh", sr_pen_sigma = 0.5, sr_R0_spec = "average", init_age_strc = 1,
      equil_init_age_strc = 2, t_spawn = 0, use_rinit = 0)),
    "shared, est, or rinit")
...
test_that("sel_norm_bins sets which bins the nonparlog standardization averages over", {
  ...
  bins <- seq_along(pars)
  sel <- function(nb) SPoRC:::Get_Selex(Selex_Model = 9, TimeVary_Model = 0, pars = pars,
...
test_that("sel_norm_bins defaults to every bin through the setup functions", {
  ...
  il <- Setup_Mod_Dim(years = 1:5, ages = 1:8, lens = NA, n_regions = 1, n_sexes = 1,
                      n_fish_fleets = 1, n_srv_fleets = 1, n_seas = 1, n_pop = 1,
                      natal_region = 1, verbose = FALSE)
