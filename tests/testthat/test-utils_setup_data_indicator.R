...
make_data <- function(n_cohorts = 4, use_tagging = 1) {
  ...
  for(data_name in fish_agg) d[[data_name]] <- array(1, dim = c(n_regions, n_years, n_seas, n_fish_fleets))
  for(data_name in fish_pop) d[[data_name]] <- array(1, dim = c(n_pop, n_regions, n_years, n_seas, n_fish_fleets))
  for(data_name in srv_agg) d[[data_name]] <- array(1, dim = c(n_regions, n_years, n_seas, n_srv_fleets))
  for(data_name in srv_pop) d[[data_name]] <- array(1, dim = c(n_pop, n_regions, n_years, n_seas, n_srv_fleets))
...
yr_dim_of <- function(data_name) if(grepl("_pop$", data_name)) 3 else 2
...
test_that("the default call zeroes every indicator in the named years", {
  ...
  for(data_name in all_names) {
    yd <- yr_dim_of(data_name)
    expect_true(all(year_slice(out[[data_name]], c(4, 5), yd) == 0), info = data_name)
    expect_true(all(year_slice(out[[data_name]], 1:3, yd) == 1), info = data_name)
  } # end name loop
})
...
test_that("only the requested data types are modified", {
  ...
  for(data_name in setdiff(all_names, c("UseCatch", "UseSrvIdx"))) {
    expect_true(all(out[[data_name]] == 1), info = data_name)
  } # end name loop
})
...
test_that("years outside the model range are ignored", {
  ...
  for(data_name in all_names) {
    yd <- yr_dim_of(data_name)
    expect_true(all(year_slice(out[[data_name]], 4, yd) == 0), info = data_name)
    expect_true(all(year_slice(out[[data_name]], c(1, 2, 3, 5), yd) == 1), info = data_name)
  } # end name loop
})
...
test_that("array shapes are preserved apart from the tagging cohort axis", {
  ...
  for(data_name in all_names) {
    expect_equal(dim(out[[data_name]]), dim(d[[data_name]]), info = data_name)
  } # end name loop
})
...
test_that("withholding every year zeroes all indicators", {
  ...
  for(data_name in all_names) {
    expect_true(all(out[[data_name]] == 0), info = data_name)
  } # end name loop
