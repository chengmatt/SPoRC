...
0 (`"equil"`) == deterministic equilibrium; 
1 (`"stoch_no_plus"`) == stochastic for all ages except the plus group; 
2 (`"stoch_all"`) == stochastic for all ages including the plus group; 
3 (`"stoch_shared_ages"`) == stochastic with user-defined age sharing via `init_age_devs_shared` |
...
| init_age_devs_shared | Integer vector of length `n_ages - 1` specifying explicit 
age-sharing for `ln_InitDevs`. Positions with the same value share a single estimated 
parameter (e.g. `c(1:42, rep(42, 9))` shares the last 9 ages with age 42, giving 42 
free parameters). Required when `equil_init_age_strc = 3`; `NULL` (default) uses 
standard behaviour. |
