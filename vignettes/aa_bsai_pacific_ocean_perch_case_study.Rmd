...
constructed and held fixed here. That is why the assessment's maturity
...
Only the Aleutian Islands survey carries a catchability prior. Supplying a
...
Beyond the data weights, the assessment carries smoothness penalties on the
...
it carries no smoothness penalty and `srv_sel_pen_wts` is `NULL`.
...
of the lognormal rather than from $\exp(\log R^{\text{init}})$. Carrying both
...
biomass and carries the $\exp(-Z t^{\text{spawn}})$ discount, while the
...
proportions carried through an applied sample size of $224$. That is consistent
...
for $1.068451$ of it. Those recruits are deviation free, so they carry the level
...
1964 and past the last selectivity node, so a handful of years carries the whole
