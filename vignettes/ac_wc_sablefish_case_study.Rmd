...
Seven conventions hold the whole bridge, and each of them can easily forgotten if missed.
...
$S_{0}$ by 0.009 percent, which the Beverton-Holt then propagates into recent
...
`SPoRC` currently has no parameter for it, so it is kept at the assessment's estimate.
...
`SPoRC` holds this as `srv_idx_type = "recdev"`, a survey fleet that observes
year class strength directly and reads no part of the population.

Two fleets have two composition data sources. The trawl fishery and the West
...
biomass is numbers times fecundity, with the age 0 cell with its value in
...
The ageing error matrix maps the 71 modeled ages onto the 51 composition bins,
...
with all of $\log F$. Fixing the mean matters: with the deviations
...
makes its compositions the right expectation. This is needed because one fleet can't have two sets of composition data.
...
`FishAgeComps_Type` is where the two composition data sources show up. The three
gear fleets have sexed compositions, which is `spltRjntS`, females and males
in one vector normalized together. The three discard fleets and the trawl companion
have compositions with the sexes pooled, which is `agg`.
...
numbers decayed half a year. The standard errors already hold the extra
...
companion of the bottom trawl survey has no index, so its catchability stays out
...
`fish_sel_sex_offset = "par_apical"` is what holds the male curve. The `par`
...
offset, which multiplies the finished curve and holds the ends along with it,
...
with $\mu_{y}$ the center the recruitment penalty asserts for that year. The
...
such an index measures. Taking it from the penalty's own center is what makes
...
element of `ln_srv_q`, kept on the log scale where the assessment has
...
The comparison before anything is optimized, against a report file with six
significant digits:
...
the extra survey standard deviation is kept at the assessment's estimate,
because `SPoRC` has no parameter for it. Neither moves the fit, but the second
does mean the standard errors here do not have that source of uncertainty.
