...
$\sigma_{R} = 0.6$. The Eastern area has no catch, so its population is
...
with none below age 3, and males have none. Growth is estimated, one
...
and the spawning weight at `t_spawn`. `growth_len_lower` holds the lower
...
the mean recruitment of the years with deviations can differ from
...
years, set by the prior on the initial-age deviations rather than by data,
