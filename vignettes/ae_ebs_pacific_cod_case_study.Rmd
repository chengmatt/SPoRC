...
at length, on 121 population length bins reported to 24 five centimeter data
bins.
...
accumulating. Population lengths are 121 one centimeter bins and the
...
fishing mortality is kept at the assessment's value, since `SPoRC` has no
...
the Richards curve, which generalizes von Bertalanffy with a sixth parameter
...
bounds. `growth_tv_type = "cohort"` has size at age forward cohort by
cohort. The deviations' standard deviations live in the first data source of
...
second data source belongs to the semi-parametric surface and is unread here.

Maturity at age is taken from the assessment rather than modeled, because
...
Two ageing error definitions are kept in `AgeingError`, a biased reader
...
The difference is the recruitment convention. The assessment declares its deviations
...
estimates it, so how much weight the index has is settled alongside
everything else. `SPoRC` has no estimated extra standard deviation on an index,
so here it is added to the observed standard errors and kept at the assessment's
...
unrecognizable without it. Holding it costs nothing at the assessment's estimate,
