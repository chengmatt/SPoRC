...
time-varying growth kept cohort by cohort.
...
informative about size at age and has little information about abundance.
...
transition, on a square root color scale.*

Two parameters hold the spread, $CV_{1}$ and $CV_{2}$ at the two reference
...
an estimated rate, and it sets how much weight the older and larger fish have.
...
Deviations outside the range are kept at zero. Use the years in which the
...
the first data source of `growth_pe_pars`, one slot per growth parameter. Fixing
...
year's curve. Nothing is kept between years: if $K$ drops, every age is
...
`"cohort"` has size at age forward instead. Writing $g_{y}$ for the growth
...
gave them, so a cohort born in a poor year has that start forward. The plus
...
to tape; and $c(x)$ is evaluated once from the first year's curve and then kept
...
parametric part and $\varepsilon$ holds the deviations from it. This represents
...
error has to borrow strength across neighboring cells. `"iid"` provides none of
...
and the 3D forms are available to the surface and not to the parametric data source,
...
parametric data source; the surface multiplies in afterwards and is not kept
forward. And parametric deviations keep size at age inside the curve's shape,
whereas the surface lets it leave.

The two data sources do share one array, `growth_pe_pars`: slot `[,,,,1]` holds the
...
Both data sources can be integrated out with the Laplace approximation instead of
...
- `vignette("ae_ebs_pacific_cod_case_study")` for Richards growth with
  time-varying parameters kept cohort by cohort.
- `vignette("n_single_region_ebs_pollock_randomeff_case_study")` for the
  random-effects routines the growth deviations share with selectivity.
