...
carries process error correlated across those regions. However, in some cases, spatial models may not be feasible. The purpose of this vignette is to demonstrate how state-space treatment of numbers-at-age can in some cases, approximate spatial process error (e.g., through migration). Let us first set up a simulation in the following with three regions, 40 years, and 8 ages.
...
hand. What is still worth carrying over is the fixed effects, taken from a
...
the operating model carries none at all. Every model in this case converged.
...
deviation is estimated at exactly zero when the operating model carries no process
error at all.

```{r, echo = FALSE, eval = TRUE, out.width = "100%", fig.cap = "Recovery of the process error parameters over thirty replicates per structure. Diamonds mark the operating model's true values. (a) the process error standard deviation, (b) the age and year correlations, (c) spawning biomass error with the state on and off."}
knitr::include_graphics("figures/naa_state_recovery.png")
```
