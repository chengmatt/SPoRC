...
modeled period, for example when bridging an assessment that carries its own
...
Under `"est_all"` each sex carries its own curve
...
carries the same compensation ratio as a Beverton-Holt at the same $h$.
...
under global density dependence) and carrying no recruitment deviation.
...
-   `"shared"` (the default) takes the scale from $\mu_p^{\text{Rec}}$,
    ...
    itself. A single parameter carries both the recruitment level and
...
-   `"est"` gives the curve its own estimated scale
    ...
    reproduces templates that carry separate mean-recruitment and
...
deterministic step becomes a prediction they are scored against.

Write $\widehat{N}_{p,r,y,a,s}$ for what the mortality and ageing step of the
previous section produces at the start of year $y$, with the plus group already
accumulated. The state $\ln N_{p,r,y,a,s}$ replaces it outright, and the
difference between them is the process error:
...
beyond bookkeeping. Because $\eta_{y}$ depends on the states in years $y$ and
$y-1$ and on nothing earlier, each term of the resulting prior is supported on a
two-year block, and the random-effects Hessian is block-tridiagonal in year. A
deviation parameterization, where the level is reconstructed by accumulating
...
`NAA_re_years`, each of which must be a contiguous run. Outside them the numbers
...
size. Rather than a variance per age, two parameters carry it: $CV_{1}$ and
...
value is held at $CV_{1}$, from $A_{2}$ on it is held at $CV_{2}$, and between
...
The two differ in what is held constant across ages. Under `"cv"` a constant
...
it sets how much weight the older, larger fish carry. And over those years mean
...
Varying the parameters. Any growth parameter can carry a deviation series
...
Under `"cohort"` size at age is carried forward instead, so each cohort keeps
...
born in a poor year carries that start forward. The plus group is the only
...
it. And the coefficient of variation at age is held at the first year's,
$c(x)$ being evaluated once from the first year's curve and parameters and then
held while the mean moves, so the deviations change mean size without also
changing the spread.
...
range entirely. That is what makes each column a proper conditional
...
so the weight at age carries the spread of length at age rather than being the
...
$p_{1}$ is the bin at which the plateau begins, carried on the bin scale rather
...
plateau sits at, which is one unless a sex carries an apical offset (see Sex
...
the difference between windows is carried by the selectivity parameters
...
scalar, and the two windows give genuinely different fits.
...
Any functional form can additionally carry a plateau (the `_NSelBins_<n>`
suffix on the model string): bins beyond a chosen bin $n^{\text{sel}}$ are
held at that bin's computed value rather than evaluated through the form,
...
scale offset carries $p_{5}$ and $p_{6}$ down with it while an apical offset
leaves them alone:
...
Each stream is stored over regions and sexes whatever a fleet reports, and the
fleet's Type in that year, $\mathcal{R}_{y,f}, \mathcal{S}_{y,f}$, names which of those margins it
reports separately: a split margin is a single index, a summed margin is the
...
single population being compared. The other two streams differ only in the
quantity summed:
...
survey-available numbers at age. The survey index carries no separate $q$: an index fit
...
index for the population-specific streams, and are shared through `rho_*_spec`.
...
The weight $\lambda_{r,y,\tau,f}$ is the one the aggregated stream carries,
...
arrays carry one age dimension shared across fleets. The length analog
...
that never encounters the youngest ages, whose zeros would otherwise carry
information).
...
those compositions carry becomes the ratio within $\mathcal{B}$. Where the
...
together with the marginal length compositions, the lengths carry the
abundance signal and the conditional rows carry $P(a \mid l)$, which is the
...
`_selex_penalty` table). For a set $\mathcal{P}$ of parameters held on
...
carries its own weight $\lambda^{\text{InitRec}}$ (`Wt_Init_Rec`),
...
(`init_age_devs_shared`), is one parameter and carries one penalty. Each cell
...
objective unweighted with $\sigma_{\text{InitSex}}$ held as data rather than a parameter.
...
zero-mean Gaussian prior whose covariance factorizes across margins, so a
correlation over one margin says nothing about any other:
...
matrix with a unit diagonal, so only $\sigma^{\text{NAA}}$ carries scale, and
...
the numbers at age and a surface on selectivity are scored the same way. The
three-dimensional field is the only one that does not factor into an age density
times a year density, because its cohort term couples the two margins.

**Population, region and sex.** $C^{\text{pop}}$, $C^{\text{reg}}$ and
$C^{\text{sex}}$ are set with `NAA_re_pop`, `NAA_re_region` and `NAA_re_sex`,
each either the identity or unstructured. Unstructured places no shape on how the
levels covary, at $n(n-1)/2$ parameters, and is the natural choice for margins
with no ordering: regions are not in a line, so a correlation decaying with
distance has nothing to decay along. `NAA_re_region_spec` sets how the region
correlations are shared over populations and sexes.
...
$\mathbf{z} = \mathbf{L}^{-1}\boldsymbol{\eta}$ leaves that margin independent,
so each slice is then scored by whatever $R^{ay}$ calls for, and the log
determinant of $\mathbf{L}$ enters once per cell of the margins it does not span.
That is what lets a cross-region correlation compose with the three-dimensional
...
since every other form is separable or Markov in one of those margins and a
...
appear in the reverse of the array's margin order, which is worth stating only
...
whitens the margins one at a time in the order the array is dimensioned.
...
carrying a weight $w$ on the squared residuals corresponds to
...
already carried by $\mu_{r,\tau,f}^{\text{Fsh}}$, so `"own_mean"` avoids
...
Growth carries two deviation surfaces, and both are scored by the same process
error machinery the selectivity deviations use, so the vocabulary above carries
over unchanged.
...
with one $\sigma_{k}$ per varying parameter $k$, held in the first stream of
...
`growth_tv_years` carry a deviation; the rest are held at zero, so a parameter
...
second stream of `growth_pe_pars`, in the same slots the selectivity forms use,
and the
...
enabled, each enter as their own additional component carrying no weight
of their own.
