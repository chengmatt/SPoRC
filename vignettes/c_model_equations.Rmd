...
Catch, discards and the fishery and survey indices can each be fit at age rather
than aggregated with a composition alongside. Every age is its own observation
with its own standard deviation, and for the indices its own catchability.

Each stream is stored over regions and sexes whatever a fleet reports, and the
fleet's Type $\mathcal{R}_{f}, \mathcal{S}_{f}$ names which of those margins it
...
$\{1,\dots,n_{r}\}$ when it sums over them, $\mathcal{S}_{f}(\varsigma)$ likewise
for sexes, and $u_{f} \in \{0,1\}$ selects abundance or biomass through the
...
dead discards to the total discarded, $w$ weight at age, $N$ numbers at age,
$S^{F}$ and $S^{R}$ fishery selectivity and retention, and $I$ the
survey-available numbers at age. The indices carry no separate $q$: an index fit
