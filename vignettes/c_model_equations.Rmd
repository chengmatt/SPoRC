...
modelled period, for example when bridging an assessment that carries its own
...
recruitment there and the residual would be penalised twice.
...
penalized), or `Use_F_pen = 0`. An `"iid"` or `"ar1"` penalty centred on
...
the centre its penalty asserts (see Recruitment). The deviation relative to that
centre is the anomaly, how strong the year class was against what the model
...
cancelled by mean standardization, the scale offset is refused for the
...
than aggregated with a composition alongside. Every age is its own lognormal
observation with its own standard deviation, and for the indices its own
catchability. The log-likelihood for observed catch at age is
...
where $C$ and $D$ are retained catch and discards at age, $w$ is weight at age,
$N$ is numbers at age, $S^{F}$ and $S^{R}$ are fishery selectivity and
retention, and $I$ is the survey-available numbers at age. Catch and discards
follow the fleet's units, abundance or biomass. The population-specific forms
drop the sum over $p$ and compare a single population's prediction. The weight
$\lambda_{r,y,\tau,f}$ is the one the aggregated stream carries, applied after
summing over ages within a cell.

The standard deviations $\sigma_{a,f}$ and the catchabilities $q_{a,f}$ are
coupled through integer key matrices over age and fleet, in which equal entries
share a parameter. A single structure therefore gives one standard deviation per
age, one per age group, or one per fleet.

This is not the same statement as an aggregated observation with a composition
beside it. That factorisation is exact for a multinomial over Poisson counts,
...
(`"1dar1"`) with a correlation $\rho_{f}$ estimated per fleet, in which case the
residual vector for that cell is multivariate normal with

$$\Sigma_{ij} = \sigma_{i,f} \, \sigma_{j,f} \, \rho_{f}^{|i-j|}$$

evaluated as a Gaussian Markov random field rather than by forming $\Sigma$.
A cell with one observed age has no correlation to describe and falls back to
independent.
...
arrays carry one age dimension shared across fleets. The length analogue
...
`"own_mean"` centre is then pooled over every penalized cell across ages
...
Under the bias ramp the centre of the initial age deviation on age $a$ is the
...
modelled years show. An equilibrium recruitment stands for an average over
...
neighbouring ages and years share information. Its hyperparameters live in the
