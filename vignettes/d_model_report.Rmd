...
Reported whenever the model is built, and zero everywhere unless that data source is
fit. Each has an age dimension the aggregated counterparts do not, and is
...
The age dim is the observed ages, the columns of the fleet's ageing error matrix,
which are the model ages under the default identity matrix. A data source summed
over regions or sexes writes into slot one of those dims and
...
and `ln_sigmaSrvIdxAA`, each over observed age, sex and fleet. The age
shape of catchability is not among them: an index fit age by age has it in
