...
| `ln_sigmaC`   | Log-scale standard deviation for the catch likelihood                    | `n_regions × n_years × n_seas × n_fish_fleets`                                               |
...
| `PredCatch`   | Predicted catch (summed across populations)                              | `n_pop × n_regions × n_years × n_seas × n_fish_fleets`                                       |
| `PredFishIdx` | Predicted fishery index (summed across populations)                      | `n_pop × n_regions × n_years × n_seas × n_fish_fleets`                                       |
| `fish_sel`    | Fishery age-based selectivity                                            | `n_regions × (n_years + n_proj_yrs_devs) × n_ages × n_sexes × n_fish_fleets`                 |
| `fish_q`      | Fishery catchability                                                     | `n_regions × n_years × n_fish_fleets`                                                        |

## Survey Processes
| Name         | Description                                             | Dimensions                                                                                     |
| ------------ | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| `PredSrvIdx` | Predicted survey index (summed across populations)      | `n_pop × n_regions × n_years × n_seas × n_srv_fleets`                                         |
| `srv_sel`    | Survey age-based selectivity                            | `n_regions × (n_years + n_proj_yrs_devs) × n_ages × n_sexes × n_srv_fleets`                   |
...
Note: Length-based selectivity arrays (`fish_sel_l`, `srv_sel_l`) are only reported when `Selex_Type = 1` and share the same dimension structure as their age-based counterparts, replacing `n_ages` with `n_lens`.
...
| `move_devs`       | Movement deviations by population, region pair, year, age, and sex (including projection years)              | `n_pop x n_regions × (n_regions - 1) × (n_years + n_proj_yrs_devs) × n_ages × n_sexes`        |
...
| Name                | Description                                                   | Dimensions                                                        |
| ------------------- | ------------------------------------------------------------- | ----------------------------------------------------------------- |
| `Catch_nLL`         | Negative log-likelihood for catch                             | `n_regions × n_years × n_seas × n_fish_fleets`                    |
| `FishIdx_nLL`       | Negative log-likelihood for fishery index                     | `n_regions × n_years × n_seas × n_fish_fleets`                    |
| `SrvIdx_nLL`        | Negative log-likelihood for survey index                      | `n_regions × n_years × n_seas × n_srv_fleets`                     |
| `FishAgeComps_nLL`  | Negative log-likelihood for fishery age compositions          | `n_regions × n_years × n_seas × n_sexes × n_fish_fleets`          |
| `SrvAgeComps_nLL`   | Negative log-likelihood for survey age compositions           | `n_regions × n_years × n_seas × n_sexes × n_srv_fleets`           |
| `FishLenComps_nLL`  | Negative log-likelihood for fishery length compositions       | `n_regions × n_years × n_seas × n_sexes × n_fish_fleets`          |
| `SrvLenComps_nLL`   | Negative log-likelihood for survey length compositions        | `n_regions × n_years × n_seas × n_sexes × n_srv_fleets`           |
| `M_nLL`             | Negative log-likelihood (prior) for natural mortality         | scalar                                                            |
| `Fmort_nLL`         | Negative log-likelihood (penalty) for fishing mortality devs  | `n_regions × n_years × n_seas × n_fish_fleets`                    |
