...
conceptually independent of the mean F of the modelled period.
...
as `spltRspltS`, which normalises each sex to sum to one separately, so no
...
The fixed-gear fleet is logistic, parameterised by the age at 50% selection and
a slope:
...
The trawl fleet is dome shaped, using the reparameterised gamma form in which
...
Parameter sharing across sexes and blocks stabilises the fit and cannot be
...
`SPoRC` parameterises sex-specific natural mortality directly rather than as an
