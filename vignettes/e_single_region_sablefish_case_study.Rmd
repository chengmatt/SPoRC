...
and scales the F series. This is the assessment's own construction, so it is
...
under a lognormal prior, with males with an offset $\delta^{M}$ from the
...
of squares and the assessment's weights are reused unchanged as `Wt_Catch = 50`
and `Wt_F = 0.1`.
...
Data sources have a $\lambda$ weight that multiplies the whole component
