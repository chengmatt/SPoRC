...
against, ships with the package in `sgl_rg_ebswp_data`.
...
The object has three kinds of content: the model inputs
...
with $\alpha$ and $\beta$ reparameterized through steepness $h$ and unfished
recruitment $R_0$. Steepness is estimated under a beta prior. In the 2024
assessment that
prior sits on the unrescaled $(0,1)$ support and is symmetric, so its center is
...
stock recruit residuals are penalized over the years 1978 to 2022, excluding
1979:

$$\ell^{\text{Rec}} = \sum_{y \in \mathcal{Y}^{\text{SR}}} \dfrac{\left(\epsilon_{y}^{\text{Rec}}\right)^{2}}{2\sigma_{R}^{2}},\qquad \sigma_{R} = 1$$

The initial numbers at age have weight $0.1$ and the recruitment level weight
...
derived from an equilibrium, and their penalty is centered on their own mean.
...
value penalized about its own mean. In `SPoRC` that is `ln_F_mean_spec = "fix"`,
which pins the mean at zero and maps it off so the deviations have all of
...
the 2024 assessment normalizes the acoustic compositions over ages 2--15 only, which is
`SrvAgeComps_bins`.
...
have their own process error, which no weight multiplies, so the trawl survey's
...
Beyond the data weights, the 2024 assessment has a set of shape penalties on
...
the coefficients and has to be kept at zero, and bins within a group must move
...
flat along it. The 2024 assessment's `avgsel` penalty does this:
...
Before optimizing anything, check that the objective is
...
model can exceed one, so the values are normalized to their maximum to make the
shape readable.
