...
1) Beverton-Holt Recruitment, where steepness is fixed at the 2024 assessment estimated value,
...
5) Initial age structure is derived by assuming a geometric series (`init_age_strc = 2`; the alternative is iterating age structure to some equilibrium `init_age_strc = 0`), and 
...
To set up fishery indices and composition data, we will supply arrays of fishery indices and their associated standard errors (`ObsFishIdx` and `ObsFishIdx_SE`), which are dimensioned by `n_regions`, `n_years`, `n_seas`, `n_fish_fleets`. Additionally, we will supply both fishery age composition data and length composition data. Both `ObsFishAgeComps` and `ObsFishLenComps` are dimensioned by `n_regions`, `n_years`, `n_seas`, `n_bins (n_ages | n_lens)`, `n_sexes`, `n_fish_fleets`. Because the Bering Sea pollock model does not fit to length composition data, `ObsFishLenComps` is specified as `NA` and `UseFishLenComps` is specified with 0s. Additionally, we specify that the fishery index is biomass-based, multinomial likelihoods are utilized for age composition data, and fishery age compositions are aggregated across regions and sexes (because this is a single-region single-sex model).
...
Setting up survey indices and compositions follows a similar fashion. Here, we will supply arrays of survey indices and their associated standard errors (`ObsSrvIdx` and `ObsSrvIdx_SE`), which are dimensioned by `n_regions`, `n_years`, `n_seas`, `n_srv_fleets`. In this case, we are modelling three survey fleets, where fleet 1 is the bottom trawl survey, fleet 2 is the acoustic trawl survey, and fleet 3 is the acoustic vessel of opportunity survey. We will also input both survey age composition data and length composition data. Both `ObsSrvAgeComps` and `ObsSrvLenComps` are dimensioned by `n_regions`, `n_years`, `n_seas`, `n_bins (n_ages | n_lens)`, `n_sexes`, `n_srv_fleets`. All three survey indices are biomass-based and multinomial likelihoods are used to fit to age-compositions for fleets 1 and 2. However, because the acoustic vessel of opportunity survey does not have composition data, these are not fit to (and hence specified as none).
...
Next, we can extract out parameter estimates of recruitment and spawning biomass and compare to the 2024 pollock model. Since `Rec` and `SSB` are both dimensioned `n_pop × n_regions × n_years`, we rename the melted dimensions accordingly.
