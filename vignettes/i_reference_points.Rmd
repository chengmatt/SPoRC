...
where $Z_{p,r,a,\tau}$ includes both retained and dead discard components as defined above, and the product is ordered sequentially across seasons. The plus-group equilibrium vector satisfies $\mathbf{N}_{p,a_+} = \mathbf{T}_{p,a_+} \mathbf{N}_{p,a_+} + \mathbf{T}_{p,a_+-1} \mathbf{N}_{p,a_+-1}$, which rearranges to the linear system:
...
The inputs, per-recruit accounting, seasonal mortality, movement, and analytical plus-group solution for global $MSY$ follow the same structure as described under global $SPR$ above, with $F_x$ replaced by $F_{msy}$. Additionally, catch-at-age per recruit is accumulated each season using Baranov's catch equation with only the landed fishing mortality:
...
For multi-population models, population $p$ recruits only into its own natal region. The same seasonal movement dynamics, mortality (with retained + dead discard decomposition), and analytical plus-group solution described under global $SPR$ and global $MSY$ are applied, with cohorts tracked by both origin $o$ and destination $r$. Catch-at-age per-recruit uses only the landed fishing mortality from non-discard fleets, with $F_{r,msy}$ now region-specific:
