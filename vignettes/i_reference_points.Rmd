...
Subsequent numbers at age per-recruit are computed with a seasonal exponential mortality model. Within each season $\tau$, the seasonal total mortality is defined as:

$$Z_{p,a,\tau}^{fished} = \text{Natmort}_{p,a,s=1} \cdot \Delta\tau_\tau + \sum_{f}^{}\text{Frat}_{f,\tau} \cdot F_{x} \cdot \text{Sel}_{a,s = 1,f}^{\text{Fsh}}$$
...
The plus group ($a_+$) is solved analytically using the scalar geometric series, which is appropriate for the non-spatial case:
...
where $Z_{p,a_{+}}^{fished}$ and $Z_{p,a_{+}}^{unfished}$ are the annual total mortality rates for the plus group, accumulated across all seasons. $N_{a}^{\text{fished}}$ can then be converted to fished $SSBPR$ with the following equation, where a mid-season spawning correction $t^{spawn}$ is applied:
...
$$\text{effSB}_{p_2} = SS{BPR}_{p_2}^{fished} + \sum_{p \neq p_2} \phi_p \cdot SS{BPR}_{p}^{fished}$$

$$\text{effSB0}_{p_2} = SS{BPR}_{p_2}^{unfished} + \sum_{p \neq p_2} \phi_p \cdot SS{BPR}_{p}^{unfished}$$

where $\phi_p$ is the stray rate of population $p$. The $SPR$ rate is then defined as:
...
-   seasonal recruitment proportions ($\chi_{p,\tau}$),
-   stray rates between populations ($\phi_p$),
-   an estimate of virgin recruitment ($\mu_p^{Rec}$), and
-   an estimate of steepness ($h_p$).
...
The numbers-at-age are decremented following the same seasonal exponential mortality model as described for $SPR$ above, with $F_x$ replaced by $F_{msy}$. Catch-at-age per recruit is accumulated across seasons using Baranov's catch equation:

$$C_{p,a,\tau} = N_{p,a}^{fished} \cdot \frac{\sum_{f}^{}\text{Frat}_{f,\tau} \cdot F_{\text{msy}} \cdot \text{Sel}_{a,s = 1,f}^{\text{Fsh}}}{Z_{p,a,\tau}^{fished}} \cdot \left( 1 - \exp\left\lbrack - Z_{p,a,\tau}^{fished} \right\rbrack \right)$$
...
-   the relative fishing mortality between fishery fleets and seasons
    ($Frat_{r,f,\tau} = Fmort_{r,y = n_{y},\tau,f}/\sum_{f}^{}{Fmort_{r,y = n_{y},\tau,f}}$),
-   fleet-specific fishery selectivity
    ($\text{Sel}_{r,y = y^{*},a,s = 1,f}^{\text{Fsh}}$),
-   natural mortality for females
    ($\text{Natmort}_{p,r,y = y^{*},a,s = 1}$),
-   spawning weight at age for females
    ($W_{p,r,y = y^{*},\tau^{spawn},a,s = 1}^{spawn}$),
-   maturity at age for females ($Mat_{p,r,y = y^{*},\tau^{spawn},a,s = 1}$),
...
For seasons $\tau > 1$ and age $a = 1$, additional recruits are added proportional to $\chi_{p,\tau}$. Following the initialization of these quantities, seasonal movement dynamics are applied within each season before mortality:
...
When $n_\tau = 1$ and $n_p > 1$ (single-season natal homing), a separate spawning movement matrix $\mathbf{M}^{spawn}_{p,a,s=1}$ is applied to redistribute fish to natal grounds before computing spawning biomass:
...
where the product is ordered sequentially across seasons. The plus-group equilibrium vector satisfies $\mathbf{N}_{p,a_+} = \mathbf{T}_{p,a_+} \mathbf{N}_{p,a_+} + \mathbf{T}_{p,a_+-1} \mathbf{N}_{p,a_+-1}$, which rearranges to the linear system:
...
in a similar manner. When multiple populations are modeled, effective spawning biomass per recruit at each population's natal region accounts for stray contributions:

$$\text{effSB}_{p_2} = SS{BPR}_{p_2, r^{natal}_{p_2}}^{fished} + \sum_{p \neq p_2} \phi_p \cdot SS{BPR}_{p, r^{natal}_{p_2}}^{fished}$$
...
The inputs, per-recruit accounting, seasonal mortality, movement, and analytical plus-group solution for global $MSY$ follow the same structure as described under global $SPR$ above, with $F_x$ replaced by $F_{msy}$. Additionally, catch-at-age per recruit is accumulated each season using Baranov's catch equation:

$$C_{r,a,\tau} = N_{r,a}^{\text{fished}} \cdot \frac{\sum_{f}^{}\text{Frat}_{r,f,\tau} \cdot F_{\text{msy}} \cdot \text{Sel}_{r,a,s = 1,f}^{\text{Fsh}}}{\left( \text{Natmort}_{r,a,s = 1}\Delta\tau_\tau + \sum_{f}^{}\text{Frat}_{r,f,\tau} \cdot F_{\text{msy}} \cdot \text{Sel}_{r,a,s = 1,f}^{\text{Fsh}} \right)} \cdot \left( 1 - \exp\left\lbrack - Z_{r,a,\tau}^{fished} \right\rbrack \right)$$
...
For multi-population models, population $p$ recruits only into its own natal region. The same seasonal movement dynamics, mortality, and analytical plus-group solution described under global $SPR$ and global $MSY$ are applied, with cohorts tracked by both origin $o$ and destination $r$. Catch-at-age per-recruit follows Baranov's equation with region-specific $F_{r,msy}$:

$$C_{p,o,r,a,\tau} = N_{p,o,r,a}^{\text{fished}} \cdot \frac{\sum_{f}^{}\text{Frat}_{r,f,\tau} \cdot F_{r,msy} \cdot \text{Sel}_{r,a,s = 1,f}^{\text{Fsh}}}{Z_{p,r,a,\tau}^{fished}} \cdot \left( 1 - \exp\left\lbrack - Z_{p,r,a,\tau}^{fished} \right\rbrack \right)$$
...
where $\phi_{p \to p_2} = 1$ if $p = p_2$ and $\phi_{p \to p_2} = \phi_p$ otherwise.
...
sgl_ref_pt <- Get_Reference_Points(data = sgl_rg_sable_data,
                                   ...
                                   rec_age = 2,
                                   t_spawn = 0,
                                   sex_ratio_f = array(0.5, dim = c(sgl_rg_sable_data$n_pop, sgl_rg_sable_data$n_regions))
                                   )
...
WAA <- array(rep(sgl_rg_sable_data$WAA[,,length(sgl_rg_sable_data$years),,,], each = n_proj_yrs), dim = c(n_pop, n_regions, n_proj_yrs, n_seas, n_ages, n_sexes))
WAA_fish <- array(rep(sgl_rg_sable_data$WAA[,,length(sgl_rg_sable_data$years),,,], each = n_proj_yrs), dim = c(n_pop, n_regions, n_proj_yrs, n_seas, n_ages, n_sexes, n_fish_fleets))
MatAA <- array(rep(sgl_rg_sable_data$MatAA[,,length(sgl_rg_sable_data$years),,,], each = n_proj_yrs), dim = c(n_pop, n_regions, n_proj_yrs, n_seas, n_ages, n_sexes))
fish_sel <- array(rep(sgl_rg_sable_rep$fish_sel[,length(sgl_rg_sable_data$years),,,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes, n_fish_fleets))
...
fish_sel <- array(rep(mlt_rg_sable_rep$fish_sel[,length(mlt_rg_sable_data$years),,,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes, n_fish_fleets))
