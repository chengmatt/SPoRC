...
- **`factor(NA)`** at a position means that parameter is **fixed** — it is held at its value in `input_list$par` and not estimated.
...
Because fixing a parameter holds it at its current value in `input_list$par`, starting values and fixing must always be set together. If you fix a parameter without also setting a starting value, it will be held at the default value, which may not be what you intend.
...
We start from `est_all` to get the correctly dimensioned parameter array, then overwrite the map. The parameter array `ln_fish_fixed_sel_pars` has dimensions `[n_regions, n_max_sel_pars, n_max_sel_blocks, n_sexes, n_fish_fleets]` — in this example `1 × 2 × 1 × 2 × 2`. The table below summarises the intended sharing structure before we implement it:
...
map_ln_fish_fixed_sel_pars <- input_list$par$ln_fish_fixed_sel_pars

# Fleet 1 (logistic): a50 sex-specific, slope shared
map_ln_fish_fixed_sel_pars[1, 1, 1, , 1] <- c(1, 2)  # a50: unique per sex
map_ln_fish_fixed_sel_pars[1, 2, 1, , 1] <- c(3, 3)  # slope: same integer = shared
