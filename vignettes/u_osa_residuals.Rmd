...
one-step-ahead residuals. Pass the at-age name as `index_source`. This case study
fits aggregated catch, so the call below is for a model that fits catch at age:
...
Valid at-age sources are `"CatchAA"`, `"DiscardAA"` and `"SrvIdxAA"`, each with a
`pop = TRUE` variant. The returned data frame has an extra `age` column, which the
residual plots facet on.
