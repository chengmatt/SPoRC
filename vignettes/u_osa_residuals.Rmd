...
Conventional tag recovery OSA (`tag = TRUE`) — faceted by region, recovery season, fleet, and movement/tag pooling group whenever the residual data span more than one level of each. This is a bit unwieldy to inspect and would recommend inspecting the qqplot to assess goodness of fit rather than bubble plots. In general, diagnostics for spatial models and tagging data are an active area of research and we do not discuss / explore this further:
