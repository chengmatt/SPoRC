...
`do_rec_bias_ramp = 0` and the penalty is centred on zero, which is what the
...
Natural mortality is estimated under a lognormal prior centred on $0.06$ with a
...
Both fleets use the $a_{50}$ and $a_{95}$ parameterisation of the logistic,
which is `logist2`:
...
a lognormal prior centred on $1$ with a coefficient of variation of $0.45$,
